...
In the published version, approvals happen in a separate terminal: an AI request goes through `keypaste-mcp` to a `keypaste agent` you started and unlocked yourself. In source, the unlocked app itself answers `keypaste-mcp` for its vault. It lists entry names, and it asks about each credential request in its own prompt window, which releases only on Allow once or Allow for 1 hour ([approvals](approvals.md#approving-in-the-desktop-app)). Agents names the app's process and session while it does. When `keypaste agent` already holds the vault, the unlock screen names it as the owner and the app leaves it running.

The focused target is one unlock session for desktop use, MCP requests and env launches, with approval and denial in the app. In source, locking already ends that session for agents in one step: a request still waiting at the app is denied as locked and grants are cleared. Agents lists the request waiting in the prompt, the grants in force with a Revoke for each, and this session's audit history ([approvals](approvals.md#approving-in-the-desktop-app)). Agents also connects an MCP client to the unlocked vault, checks the connection and removes it ([MCP setup](mcp-setup.md#from-the-desktop-app)). In source, Env profiles launches a project through that session after you confirm the command, the directory and the variable names, and `keypaste run --session` in a terminal asks the app for a set in a prompt window of its own ([approvals](approvals.md#runs-that-ask-for-a-projects-variables)); `keypaste run` without `--session` still unlocks on its own. The earlier terminal-owned design is no longer the required architecture.
...
Env profiles shows one project at a time, chosen from the picker at the top or the sidebar's Projects, and its Run with this profile panel copies a command naming the profile, as in `keypaste run -p dev <project> -- npm start`, with the saved command in place of `npm start` once there is one. The copied CLI command opens the vault separately, asks for its password and closes it before starting the child; it does not reuse the desktop session. In source, adding `--session` and `--vault <this vault>` to it asks this app instead: a prompt window names the project, the variable names, the command and the directory, and the command starts with the set only when you press Allow once, or Allow this command for 15 minutes, which lets the same command in the same directory run again unasked until then; a protected profile such as `prod` offers Allow once only.

A project opens on its `dev` profile, which is the project group itself, and each `env/<project>/<profile>` subgroup gets a tab beside it. A matrix shows every key against every profile and marks one that is missing, unusable or holding the same value as another profile, never the value; a note too long for a narrow cell moves below the mask, and hovering a cell gives the whole sentence. An add-profile control creates a profile, and a problems list names a subgroup that is not a profile: one named `dev` or with a name that is not a profile is never read, as `keypaste env ls <project> --profiles` lists it. The `.env.keypaste` preview has its own profile toggle, and Export .env.keypaste writes the references that profile would export, never a value. Choosing a profile sends every add, replace, remove, import, Run and Open terminal to it.

In source, a project also sets where it runs on this machine: a directory and the command you would type there, saved with Save. Run opens the platform's terminal in that directory running the command, and Open terminal opens it at a prompt: `cmd.exe` on Windows, and on Linux the first of `x-terminal-emulator`, `gnome-terminal`, `konsole` and `xterm` found on `PATH`. Neither is offered on macOS. Each first shows a card naming exactly what starts, the directory and the variables' names, never their values, and starts nothing until you press Start. The set comes from the vault as saved while the app stays unlocked: a set with an expired entry or a name that cannot be exported is refused before the card appears, naming each entry, and locking while the card is up, or before the set is released, starts nothing. The values go into the terminal's environment only, so everything you run in that terminal can read them, and locking the app does not stop it or take them back. After the command exits the terminal stays open at your shell. Import .env reads a file through the same checks as `keypaste env pull`, lists each variable as new, replacing a stored value or unchanged, and writes only when you press Import. A file with any problem imports nothing, and the file is left where it was.

A project displays masked variables with Copy and Replace buttons. A new variable generates its value unless you untick Generate a value, which reveals a masked field for one you already have; Replace opens the same field for an existing variable. The Characters or Words choice is the same one the entry form offers, over the same generator. Hold a value to reveal it; only one can be visible, and releasing, switching screens or locking hides it.

In source, each entry row carries a dot and when an agent last received it: amber and "in use" while a grant or waiting prompt names it, green when it was released in the last two hours and grey before that, beside the time since, read from this vault's audit lines and what the holder released this session, never a value. The entry's pane shows its Created and Rotated dates and an Agent access card naming each client that received it, newest first, with any grant in force. Rotate asks first, then replaces the password with a generated twenty-character one, keeps the old value in the entry's history and ends every grant for the entry; the new value is not shown. It changes the vault only, so a key a provider issued still has to be rotated there. `keypaste rotate <entry>` does the same from a terminal.

In source, Sharing lists the links made from this vault with their recipient note, expiry and views, and creates one: pick an entry and field, the expiry (1 hour, 24 hours or 7 days) and the views (1, 3 or 10), optionally require a passphrase of at least 20 characters you send another way (whoever holds the link can guess it offline, so six generated words from `keypaste generate --words 6` are the safe choice), and Create and copy link encrypts the field on this machine, uploads only ciphertext and copies the link, whose key is in the fragment. Opening the screen asks the server nothing; Check status asks how many views each link has left. Revoke ends a link that may still open, and Remove forgets one that cannot. While keypaste.com's sharing is switched off (`SHARE_ENABLED` unset) the screen says keypaste.com is not accepting shares and creates nothing. Whoever holds a link can open it until its views or time run out ([T-33](../THREATS.md#t-33--whoever-holds-a-share-link-can-open-it)).

In source, Import .kdbx in the sidebar picks another KeePass file and opens a dialog over the app. It names the file's format and key derivation, asks for its password and uses a `<name>.keyx` or `<name>.key` beside it, or one you choose. Once the file is decrypted it lists each group, how many entries it holds and where it will land, under a group named after the file, with an env set that is valid and new to this vault kept at its own `env/…` path; each row can be moved or left out, and a row that cannot be copied says why and stops the import. Its recycle bin and keypaste's own groups are not copied. Import copies everything or nothing and saves, as `keypaste import` does; the switch below it instead keeps editing the file where it is, so KeePassXC and other KDBX apps stay in sync. While the dialog is open, the search and screen shortcuts do nothing.

The title bar says whether the file holds what is open: saved, unsaved changes, or changed on disk when another program saved it. The sidebar's MCP card counts the distinct clients attached to this session, and Agents lists them as cards with their label, whether they are connected and their policy (Session grants up to 1h, Ask every time or Inject only), which it writes to `clients.toml` ([policy](policy.md#narrowing-one-client)); when the file cannot be read, every agent request is refused and Reset replaces it with an empty one. Below the client cards, Scoped tokens lists the vault's CI tokens by name, scope and expiry. New token takes a name, a scope such as `read:acme-api/staging/*` and an expiry of 7, 30 or 90 days, then shows the token once with Copy. Revoke asks for confirmation, then deletes a token for good.
...
These are the native GUI prerequisites for the current packaging targets. Building from source also requires the SDK above; CI archives and the internal MSI, AppImage and `keypaste.app` include the .NET runtime but are not published installers.
...
The internal macOS `keypaste.app` comes in a zip and is unsigned and unnotarized. A copy downloaded through a browser carries `com.apple.quarantine`, and Gatekeeper is expected to block it; no run has observed what a Mac shows. After unzipping, `xattr -dr com.apple.quarantine keypaste.app` removes the attribute from the whole bundle. CI starts an unquarantined copy through LaunchServices on `macos-15`, which does not establish a person's first launch.

Upgrading is a matter for the package. The Windows MSI is a per-user major upgrade: installing a higher version replaces the lower one in place, the previous version is removed only once the new install commits, so an upgrade that fails partway leaves the one you had installed and runnable, and the MSI refuses a lower version over a higher one. An AppImage or `keypaste.app` has no installer, so an upgrade is a new copy put in place of the old one. No package holds your data: the vault, `app.toml`, `recent.toml`, `policy.toml` and `audit.jsonl` are untouched by an upgrade, and uninstalling removes the install folder and the Start menu shortcut while leaving `~/.keypaste` and your vault alone. Recorded internal-candidate checks include upgrade and interrupted-install recovery; they do not establish a public desktop installation. See [RELEASE](RELEASE.md) for their evidence.
...
A new vault opens on an empty Secrets list. Add entries there, or with `keypaste add` in a terminal against the same file.
...
6. Complete launch, unlock, all seven destinations and `Ctrl/Cmd+L` using only the keyboard.
...
13. Agents names this app's process and session while the vault is unlocked. With `keypaste agent` holding the vault first, the unlock is refused and the unlock screen names the agent. In source, a credential request from a `keypaste-mcp` configured for the vault opens the prompt window over other windows: Allow once and Allow for 1 hour do nothing for their first second, Enter and Escape refuse, and locking the app while it is up takes it down. While it is up Agents lists it with its seconds left; after Allow for 1 hour it lists the grant, Revoke removes it, and the same request prompts again. After Allow once nothing is listed and the same request prompts again.
14. Secrets lists titles and groups. Filter by a group and search for part of a title or group path; case changes still match. Selecting an entry shows a username, a URL and notes, and a row of dots where the password is.
...
19. Hold an Env profiles value to reveal it, then release to hide it. Holding another row must reveal only that row.
...
48. In Agents, connect an installed client with a label and one extra exposure. The preview names the vault, the label and the exposure; Cancel leaves `claude mcp list` or `codex mcp list` unchanged, and Run it adds exactly the previewed command. Check the connection raises the prompt window and, after Allow once, the history shows the prompted release under the label. Remove it and the client lists no keypaste. From the installed package, the registered command is the packaged `keypaste-mcp`, or the `.AppImage` file with `mcp`.
49. On Env profiles, Import .env a file holding one new and one changed variable. The preview names both and no value; Cancel leaves `keypaste env ls` unchanged, and Import adds the new one and replaces the other, whose old value is in KeePassXC's History tab. The `.env` file is still there.
