...
The focused target is one unlock session for desktop use, MCP requests and env launches, with approval and denial in the app. In source, locking already ends that session for agents in one step: a request still waiting at the app is denied as locked and grants are cleared. Launches through that session, listing requests and grants in Agent Activity, and connecting an MCP client from the app are unfinished work in [STEPS](STEPS.md); the earlier terminal-owned design is no longer the required architecture.
