...
Env Sets shows project cards with a copyable `keypaste run <project> -- ` command; an open card's copy names its profile and command, as in `keypaste run -p dev <project> -- npm start`, with the saved command in place of `npm start` once there is one. The copied CLI command opens the vault separately, asks for its password and closes it before starting the child; it does not reuse the desktop session. In source, adding `--session` and `--vault <this vault>` to it asks this app instead: a prompt window names the project, the variable names, the command and the directory, and the command starts with the set only when you press Allow once, or Allow this command for 15 minutes, which lets the same command in the same directory run again unasked until then; a protected profile such as `prod` offers Allow once only.

A card shows the `dev` profile, which is the project group itself. In source, its view model also holds the project's other profiles (the `env/<project>/<profile>` subgroups), a matrix of every key against every profile marking one that is missing, unusable or holding the same value as another profile, never the value, and the `.env.keypaste` of references the selected profile would export; choosing a profile sends every add, replace, remove, import, Run and Open terminal to it. Those controls are not drawn on the card yet. A subgroup named `dev` or with a name that is not a profile is never read and is listed as a problem, as `keypaste env ls <project> --profiles` lists it.
