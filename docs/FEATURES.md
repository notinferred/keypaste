...
Reviewed 2026-09-07 against PRODUCT v1.2; milestone mappings updated 2026-09-15 for PRODUCT v1.4 (D-0176); entry editing restated 2026-09-18 when 4.9 closed and entry history when V.2a did. This is a first-pass capability inventory.
...
| Entry history | Ordinary updates retain prior values in KeePass history, and the core reads and restores a revision. No history-reading/restoration UI, so recovering a value still needs KeePassXC. | V.2a complete, source only; V.2b adds the desktop screen. Working proposition. |
