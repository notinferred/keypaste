...
|---|---|---|
| Create and open | CLI and desktop create and open password-protected KDBX vaults through the same core. Desktop offers a picker, drag-and-drop and recent vaults. In source, every CLI verb that opens a vault also takes `--keyfile` or `KEYPASTE_KEYFILE`, opening all four KeePass keyfile forms and a vault protected by a keyfile alone; an arbitrary hashed file is named on stderr as the form one edit destroys, and a missing, empty or unreadable file, or the vault itself, is refused before the password is asked for. | Nothing creates or attaches a keyfile, so a keyfile vault must be made in KeePassXC (V.1a2); `keypaste init` and the desktop take no keyfile, and the desktop cannot open a keyfile vault at all until V.1b. Nothing records which keyfile a vault uses. Changing a master password is unimplemented. Hardware-key unlock is committed later as P.1. |
