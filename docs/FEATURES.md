...
| History | Updates retain prior revisions; desktop lists, reveals and restores them. Restoring retains the replaced value, within the history limit. | History lives in the same vault and is not a separate backup. |
| Delete | Deletion asks for confirmation and permanently removes the entry and its history. | No recycle bin, undo-delete or built-in recovery from a lost/corrupted vault file. |
