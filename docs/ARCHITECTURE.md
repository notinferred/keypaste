...
- `keypaste run` opens the vault, reads an env set, closes the vault and starts the child with the values in its environment through the same `Core/Launch/EnvLaunch.cs` as the app. It takes no claim.
