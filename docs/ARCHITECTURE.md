...
| `src/Keypaste.Cli` | The `keypaste` command. One class per verb in `Commands/`; `CliApp` owns streams and exit codes, `VaultLocator` resolves `--vault` and `KEYPASTE_VAULT`, and `VaultSession` owns prompts, `--keyfile` and opening a vault. `keypaste agent` is the terminal approver. `Styling/` decides colour and glyph fallbacks for every verb, and `Output/CliJson.cs` is the one `--json` shape. | `Program.cs`, `CliApp.cs` |
| `src/Keypaste.Mcp` | `keypaste-mcp`, the vault-free MCP bridge over stdio. Its tools are in `Tools/`; `run` is offered only under `--allow-run` and starts the approved command through `Keypaste.Core/Launch/` (D-0358). It writes the audit record of every agent request (`McpAudit.cs`); the vault's owner writes only scoped-token lines, and `keypaste share` and `keypaste token bundle` their own (D-0352). | `Program.cs` |
| `src/Keypaste.App` | The Avalonia desktop. `Session/AppVaultSession.cs` owns the unlocked vault; `ViewModels/` has one view model per screen and `Views/` its XAML; `ShellViewModel` disposes every screen on lock. `Theme/` holds the design tokens, the control styles by class and the Lucide icon geometry (generated by `scripts/lucide-to-axaml.py`). | `Program.cs`, `App.axaml.cs` |
...
- `keypaste-mcp` is started by the MCP client, never opens a vault, forwards each request to the vault's owner over the pipe and writes the audit record. Under `--allow-run` it also starts a command an agent names, after the owner's approval, with the approved set in its environment (`Mcp/Tools/RunTool.cs`, D-0358).
- `keypaste run` opens the vault, reads an env set, closes the vault and starts the child with the values in its environment through the same `Core/Launch/EnvLaunch.cs` as the app. It takes no claim. With `--session` it opens nothing: it attaches to the owner's endpoint as the bridge does and sends an env request, which `SessionAuthority` resolves through `SessionEnvResolver` under the attached session, asking through the owner's own `ApprovalGate` and channel, and starts the child with the whole set the reply carries (D-0341). With `--token` it sends a token env request the owner verifies against `Core/Tokens/` instead of asking, and with `--bundle` it opens a token bundle with no vault or pipe (D-0352, D-0353).
- `keypaste grants` and `keypaste lock` attach to the owner as the bridge does and list or end its grants, or lock it (D-0351). `keypaste share` opens the vault under its claim, seals one field and uploads the ciphertext to keypaste.com's `/api/share`, which `site/src/share.js` serves (D-0354).
...
| Share links: sealing, the keypaste.com client and the vault's records | `Sharing/` |
| Probing a KDBX header and copying another vault in (`KeePassInterop` does the clone and attach) | `Import/` |
| Starting a program with an env set: the environment merge, the platform's terminal and the launchers; for `run`, resolving the program, containing the child, capturing its output and scrubbing the values from it | `Launch/` (`RunProgram.cs`, `ChildContainment.cs`, `CapturedLaunch.cs`, `OutputScrubber.cs`) |
| Where a project runs on this machine (`projects.json`), and which project a directory is | `Projects/` |
...
| Approval, grants and credential release; the `run` request's rules and prompt | `Approval/` (`RunRequestRules.cs`, `RunPrompt.cs`) |
