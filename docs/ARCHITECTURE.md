...
| `src/Keypaste.Mcp` | `keypaste-mcp`, the vault-free MCP bridge over stdio. Its two tools are in `Tools/`. It writes the audit record of every agent request (`McpAudit.cs`); the vault's owner writes only scoped-token lines, and `keypaste share` and `keypaste token bundle` their own (D-0352). | `Program.cs` |
