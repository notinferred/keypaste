...
- The desktop app unlocks its own session (`App/Session/AppVaultSession.cs`) and locks it on idle, on `Ctrl/Cmd+L`, on quitting and, when that setting is on, on minimize. Each unlock is a `Core/Ownership/SessionLifetime.cs`, and every lock ends it before the vault is disposed, withdrawing what agents have waiting and zeroing their grants (D-0313). `App/Session/SessionHost.cs` serves the unlocked vault to agents: listings are answered, and each credential request is put to the person in the prompt window `App/Session/WindowApprovalChannel.cs` shows (`Views/ApprovalWindow.axaml`), and each `keypaste run --session` request in `Views/EnvApprovalWindow.axaml`, with no policy file consulted. `App/Session/AppAuthority.cs` is the two together as launch composes them: disposing it is quitting, and its status is read from the authority answering agents (D-0321). Env Sets launches a project's terminal through `App/ViewModels/ProjectLaunchViewModel.cs`: the session resolves the set under the confirmation it shows, and `Core/Launch/` starts the terminal detached (D-0340). Agent Activity's Connect section runs an MCP client's own registration command and starts the registered `keypaste-mcp` once to check it, both through `Core/Clients/`; the desktop packages carry that bridge (D-0334).
...
- `keypaste run` opens the vault, reads an env set, closes the vault and starts the child with the values in its environment through the same `Core/Launch/EnvLaunch.cs` as the app. It takes no claim. With `--session` it opens nothing: it attaches to the owner's endpoint as the bridge does and sends an env request, which `SessionAuthority` resolves through `SessionEnvResolver` under the attached session, asking through the owner's own `ApprovalGate` and channel, and starts the child with the whole set the reply carries (D-0341).
...
| Env sets and dotenv | `EnvStore.cs`, `EnvConvention.cs`, `EnvNameRules.cs`, `EnvResolution.cs` (what may leave for a child), `EnvImport.cs`, `DotEnv.cs`, `DotEnvWriter.cs`, `SourceSnapshot.cs` |
| Starting a program with an env set: the environment merge, the platform's terminal and the launchers | `Launch/` |
| Where a project runs on this machine (`projects.json`) | `Projects/` |
...
- `tests/Keypaste.<Project>.Tests` mirror the projects. The helper projects `TxfContender`, `PoolStarver`, `VaultSaver`, `VaultRestorer`, `MinimizeObserver`, `AppDriver`, `FakeMcpClient` and `EnvReporter` are unshipped processes that particular probes and gates drive; `AppDriver` performs one desktop act through the screen's view model, or holds a vault unlocked and served as the app does, with the app's prompt window drawn on a headless display and clicked. `FakeMcpClient` builds as `claude` and answers Claude Code's `mcp add`, `remove` and `list`, so the connect gate never writes a real client's configuration. `EnvReporter` is the child the app's Run starts in its tests, reporting its own environment over a pipe.
