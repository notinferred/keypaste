...
The [detail model](../src/Keypaste.App/ViewModels/EntryDetailViewModel.cs) applies [name sanitization](../src/Keypaste.Core/EntryNameSanitizer.cs) to URLs and notes. `https://example.test/path` consequently displays as `https: example.test path`. Newlines, brackets and other structural characters also become spaces, damaging authentication instructions and configuration examples. Stored values and edit drafts remain unchanged. Preserve legitimate punctuation and line breaks while retaining protection against deceptive controls.
