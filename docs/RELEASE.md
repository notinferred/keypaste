...
The public release is CLI/MCP **`v0.2.0`**, served from `https://dl.keypaste.com/v0.2.0/`; `v0.1.0` is superseded and stays where it is. All four archive URLs returned HTTP 200 on this date; that availability check is not a new installation verification. `main` contains the [0.2.0-rc.1](../CHANGELOG.md#020-rc1) changes, including `setup`, and `Directory.Build.props` declares `0.2.0`; neither is in the download above.
...
- [`verify-release-matrix.sh`](../scripts/verify-release-matrix.sh) holds `release-targets.json`, the workflows, the csprojs and the download pages to one answer, against its fixture suite and a negative control; [STEPS R.0a](STEPS.md) records how many cases ran and what each refused. It runs in `ci.yml` and in `release.yml`'s guard. **It reads `site/public/index.html` at the checked-out ref, not what keypaste.com serves**, and the two can differ. Cloudflare's Git integration deploys `site/` on a push to `main` (D-0127), and no workflow here publishes the page or asks the origin about it: [`verify-site-disclosure.sh`](../scripts/verify-site-disclosure.sh) against the live page is a by-hand check, because a job that asked keypaste.com would go red for a Cloudflare outage it cannot fix. Nothing in this paragraph is a release gate. Verifying the public origin is requirement 4's job, and `install.yml` does the part of it that needs no credential.
- [`install.yml`](../.github/workflows/install.yml) runs the README install blocks weekly or manually on all four advertised targets, each on a fresh GitHub-hosted runner. It checks the installed CLI version and the presence of MCP, and then makes the binary create a vault and inject a value into a child process, so a release that starts and cannot work is not recorded as installable. Linux ARM64 has no block of its own: the definition records `install_block`, and the substitution README instructs is what runs. A `version` dispatch input targets any entry in `published[]`, which is how an unadvertised candidate is exercised before any page points at it (D-0118). It still does not cover the subsequent setup/use instructions, desktop installation or automatic post-publication verification. A retained successful run is required to claim its result for a specific release.
...
[`verify-release-preflight.sh`](../scripts/verify-release-preflight.sh) drives the three middle rows over its fixture suite, with two negative controls that reproduce answers this repository has actually given: an unanchored changelog match taking an rc heading for the release, and an asset count without its published floor taking a release that dropped a target.
