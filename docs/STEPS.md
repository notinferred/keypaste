...
- [ ] **3.6b — Sign Windows executables and installers.** Needs: 4.7a1.
  **Build:** a signing script run from `app.yml`'s `win-x64` job and `release.yml`'s `win-x64` build Authenticode-signs and RFC 3161-timestamps `keypaste-app.exe`, the first-party DLLs, the MSI and the CLI/MCP executables through `signtool /dlib` (D-0143), reading the endpoint, account, certificate profile, OIDC client ID and tenant ID from repository variables after an `azure/login` step that runs only when the resolved policy is `authenticode`, and refuses to produce an unsigned payload when `signing.policy` asks for a signature and any of the five is absent. **Input:** those five values from 3.6a. Until they exist the same script signs with a certificate generated in the job and trusted only on that runner, `signing.policy` stays `none`, nothing signed that way is uploaded, and the dlib path stays unexercised (D-0144). `verify-release-matrix.sh` currently refuses any `signtool sign` while a policy is `none`, so the gate learns to tell the runner-trusted rehearsal from a policy change.
  **Verify (V-3.6b):** on `windows-2025` the rehearsal signs the payload and MSI, `signtool verify /pa /v` accepts each under the job's trusted root and names its timestamp, and a changed byte, a missing timestamp and a certificate outside that root are refused; a policy of `authenticode` with no identity fails the job before any artifact is written; `verify-windows-installer.sh` still requires `NotSigned` while the definition says unsigned; the matrix gate refuses a rehearsal signature reaching an uploaded artifact. The real publisher, timestamp authority and install prompts are recorded only after 3.6a supplies the input.
