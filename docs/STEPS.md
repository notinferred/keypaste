...
binary reports keypaste as its publisher on all seven built targets (F.4b). F.2b2 and F.5 are the repair tasks still open: F.2b2 is BLOCKED on hardware rather than on work,
and F.5 is a handshake race on macOS NativeAOT that a release dispatch found on 2026-09-09.
...
**Next: R.0a — pin release identity and the supported platform contract.** A runner has now seen
most of it: `ci` and `app` are green on 71ac604, and release dispatch 34387033087 expanded the
derived matrix to the same four RIDs and packaged three of them under names carrying the full
version. **The row closes on a prerelease tag and not before**, because the defect V-R.0a's last
sentence names is the desktop one and `app.yml` sets its suffix only on a tag: the dispatch logged
`prerelease suffix: <none>`. That dispatch also **found a defect of its own** — F.5, below, a
handshake race on macOS NativeAOT that failed `osx-arm64` twice in two attempts and is not from any
recent commit. Every repair that can be finished without new hardware is finished. F.2b2 is BLOCKED on a macOS machine and a Linux desktop
...
- [ ] **F.5 — Hold the handshake's ordering guarantee for the call that follows it.** Needs: F.3b.
  **Build:** A `tools/call` that arrives after `notifications/initialized` on the same connection is answered "called before the initialize handshake completed" on macOS NativeAOT. The refusal itself is correct and stays; what is wrong is that a client which completed the handshake correctly reaches it. Establish whether the notification and the request that follows it are dispatched concurrently, or whether one stdin read delivering all three messages is what reorders them, and make the handshake state a call cannot overtake. Keep the pre-handshake denial for the case it was written for, which [MCP stdio tests](../tests/Keypaste.Mcp.Tests) already cover.
  **Verify (V-F.5):** A reproduction sends `initialize`, `notifications/initialized` and `tools/call` in one write on the affected build and never receives the pre-handshake denial, red before the fix. `verify-approval-e2e.sh` and `verify-policy-e2e.sh` pass on `osx-arm64` in a release dispatch, which is where this is observable.
  **Status:** Repaired in source, unverified on the target it was seen on. [McpAudit.AwaitCompletionAsync](../src/Keypaste.Mcp/McpAudit.cs) re-reads the identity for a bounded second before refusing, so a client whose `initialize` is still being processed is waited for and a client that sent nothing is still refused — the guarantee is unchanged, a request is never answered for a client that cannot be named. [Grace tests](../tests/Keypaste.Mcp.Tests/HandshakeGraceTests.cs) drive a predicate that flips the way `ClientInfo` does; deleting the wait turns two of them red. **The ordering itself cannot be reproduced in process** — every in-process test drives a real `McpClient`, which waits for the handshake response — which is why this reached a release runner twice. **It had been seen once before and was not treated as a defect:** `verify-mcp-stdio.sh`'s phase 7 comment records it as "a real defect, observed on a macOS runner during a release build" and the response was to make that gate wait for the initialize response, leaving `verify-approval-e2e.sh` and `verify-policy-e2e.sh` pipelining all three messages and losing the race. Observed failing twice in two attempts of release dispatch 34387033087: the first at the no-agent probe (id 2), the re-run at the policy gate (id 5), which is why it is filed as a race. Not a regression from any recent commit — 979 changed lines across 24 `src/*.cs` files since the last green dispatch, none non-comment. **Closing needs `osx-arm64` green in a release dispatch**, and green twice rather than once, because two attempts is what it took to see it twice. Alternative not explored: pinning the SDK's dispatch order rather than waiting for what it publishes.
