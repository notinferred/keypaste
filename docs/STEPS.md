...
binary reports keypaste as its publisher on all seven built targets (F.4b). F.2b2 is the one
repair task still open, and it is BLOCKED on hardware rather than on work.
...
**Next: R.0a — pin release identity and the supported platform contract.** Its artifacts are built
and its gate is green here: what keypaste ships is one checked file the workflows build from rather
than a list in seven places. **The row is not closed, because none of it has run on a runner.** The
derived matrices are `fromJSON` expressions, which is the one part of this work no local run can
exercise at all, and the first push after 2026-09-07 found three defects no local suite could see.
A `ci` run and a `release.yml` dispatch close it. Every
repair that can be finished without new hardware is finished. F.2b2 is BLOCKED on a macOS machine and a Linux desktop
