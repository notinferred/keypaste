...
This revision marks V.4a complete and selects no further implementation task. The founder decides what gets built next. On a later instruction to build the next task, follow the track order and the first ready code row; a specific task selection takes precedence. No backlog item is automatically eligible.
...
|---|---|---|---|
| T1 | Everyday vault use and recovery | A person opens or creates a supported vault, organizes and uses credentials, restores an edit or deletion, recovers a backup and changes vault access settings from the app. | Creation, editing, history, generators, deletion recovery and whole-file backups exist; showing and restoring a backup, organization controls, access settings and current-password reveal remain. |
...
Recovery comes before broader feature depth. V.3a repaired the delete that lost an entry and its history, V.3b made that recovery reachable, and V.4a made the file-level copy a recycle bin cannot give; what remains is reaching those copies from the app, and the organization and access settings daily use needs. These five tasks are the next detailed implementation candidates, not work started by this document revision. Each uses the existing shared core and traces to PRODUCT §§1, 2 and 5.7.

- [ ] **V.4b — Restore and export an encrypted vault from the app.** Needs: 4.8.
...
- [ ] **V.1b — Manage vault access from the desktop.** Needs: V.1a, 4.8. — Choose an existing keyfile and change the master password/keyfile through the shared core, with confirmation and a tested reopen path; no hosted recovery claim.
- [ ] **V.10 — Complete current-password use and accessible input.** Needs: 4.9. — Hold-to-reveal for a selected current password, consistent copy/clear behavior and constant accessible names on secret inputs; exercise real drawing and automation without exposing hidden values. More generator recipes remain optional.
