...
- [ ] **4.7a3 — Package an internal Linux AppImage.** Needs: R.0a, F.4b.
  ...
  **Local record:** at source VersionPrefix 0.3.1, 2026-09-16. `ubuntu:24.04` in Docker built `keypaste-app-0.3.1-rc.1-linux-x64-internal-unsigned.AppImage` (sha256 `86d70b99…`) from a staged linux-x64 publish, with `appstreamcli` validation passing; the verifier unpacked 226 files byte-identical at offset 944632, `--selftest` passed and 3 binaries publish as keypaste. A changed appimagetool or runtime pin was refused with no output, and a version the image does not carry was refused. The matrix gate refuses 46 cases. Pending: the branch dispatch and the candidate-tag run.
