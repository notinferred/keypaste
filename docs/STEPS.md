...
| R.0f | 0.3.0 published with the F repairs, and the pages moved off `0.2.0`'s defects | release run 35046281003 at `307bb54` published all 11 assets, the manifest and the Sigstore bundle; [verify-provenance.sh](../scripts/verify-provenance.sh) `0.3.0` confirmed every one anonymously from the origin. Install run 35047059579 on `ubuntu-24.04`, `ubuntu-22.04-arm`, `macos-15` and `windows-2025`, each creating a vault and injecting into a child. `0.3.0` carries no known defects; `0.2.0` and `0.1.0` keep theirs with upgrade notices; D-0195 |
