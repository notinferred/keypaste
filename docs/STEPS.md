...
`0.3.0` publishes the local vault, the CLI/env workflow and the approval bridge on all four native targets, installed from clean runners and carrying no known defects; `0.2.0` and `0.1.0` stay served with their defects disclosed. The desktop is not publishable yet: its packages build, install, upgrade and uninstall without losing user data on runners, and 4.7c1 built the path that would publish them. 4.8 gave it vault creation in source and 4.9 gave it storing and replacing a secret somebody already has, both on rules it shares with the CLI, so a person can now keep a credential without a terminal. V.2a made that history readable and restorable in the core and V.2b put it on the entry pane, so an ordinary mistake can be recovered without KeePassXC. Sharing, receiving, the drop relay and native approval are unimplemented, and F.15 records a documented unlock-screen shortcut that does nothing.

V.6 is the next ready code row, then 1.4a. Signing and publication (4.7c2) come last, gated on that desktop work and on the identity it names as its input. The macOS desktop app and all Apple signing are Expansion (D-0201). Closed rows keep their evidence in [Completed steps](#completed-steps); the reasoning is in [DECISIONS](../DECISIONS.md).
