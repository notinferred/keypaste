...
- [ ] **F.5 — Hold the handshake's ordering guarantee for the call that follows it.** Needs: F.3b.
  ...
  **Status:** Observed 2026-09-09 on **macOS + NativeAOT only**, twice in two consecutive attempts of release dispatch 34387033087, and it moved: the first attempt failed `verify-approval-e2e.sh` at the no-agent probe (request id 2), the re-run passed that gate and failed `verify-policy-e2e.sh` (id 5) with the same denial. Both Linux AOT builds, the Windows AOT build and macOS JIT under `ci` passed the same gates on the same commit. **Not a regression from R.0a or from any recent commit:** between the last green release dispatch (b799eb5) and 71ac604 there are 979 changed lines across 24 `src/*.cs` files and none of them are non-comment. That it moves between gates is why it is filed as a race rather than as a broken assertion; the mechanism is a hypothesis and is labelled one, in the same terms as the F.3b note above.
