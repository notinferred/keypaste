...
- [ ] **4.7d — Preserve user data through upgrade, uninstall and recovery.** Needs: 4.7a.
  ...
  **Pending:** run 35260785520 at `2b91815` passed `linux-x64` and failed `win-x64` in setup, before the harness: a Windows destination path made coreutils escape the digest of the pinned KeePassXC download, so the asset was refused, and the `if: always()` upload then failed the job over an empty directory, leaving that RID no records. Both are repaired: one script owns the pinned Windows KeePassXC download for all three workflows, `fetch-pinned-asset.sh` reads its digest from stdin with a backslashed-path case in its selftest, and the records directory is made before the first step that can fail while an empty upload only warns (D-0208). The interrupted install now ends `msiexec` mid-copy and passes only on a logged rollback (D-0206). The offline selftest is 39 cases; the runner evidence needs one further dispatch, and until it passes on both RIDs this row stays open, with a contradiction opening its own F row.
