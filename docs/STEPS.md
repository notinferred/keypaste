...
| F.10a | A save's wait is split into check, gate, attempts and sleeps | [SaveTimingTests](../tests/Keypaste.Core.Tests/SaveTimingTests.cs) name a held gate's holder. 30 suites, 834a9e6 plus uncommitted instrument, Windows 10 19045: 55 of 60 doomed saves gate-dominated, 0–827 ms; first attempt ≤3 ms; D-0134 |
...
- [ ] **F.10b — Gate only the saves that write into the shared temporary directory.** Needs: F.10a.
  **Measured (D-0134):** the `_saveGate` in `KeePassInterop.Save` serialises every save in the process, but only a transacted save writes into the private temporary directory D-0123 protects. KeePassLib transacts only when the vault file exists, is not a reparse point and TxF is usable. A doomed save whose directory is gone never transacts, yet queued 0–827 ms locally behind other saves' 47–450 ms attempts; CI's 3622–6275 ms first interval is consistent with a deeper queue and is still unmeasured there.
