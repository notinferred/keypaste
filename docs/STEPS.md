...
was supposed to refuse it (F.3b), a vault with more names than one reply can carry is listed and
bounded rather than dropping the connection and its grants (F.3c), and a release too large to send is refused and
said to be one instead of costing the connection, the approvals on it and a second prompt (F.3d).
A release destination is now refused unless a listing positively verifies it is empty, so a denied
or unreachable one can no longer read as free and be published over (F.4a), and every shipped
binary reports keypaste as its publisher on all seven built targets (F.4b). F.5's handshake race is repaired and `osx-arm64` is green twice on the commit a tag would ship, on
weak evidence its row states rather than hides. F.2b2 is the repair still open, BLOCKED on hardware
rather than on work.
...
**Next: R.0a — pin release identity and the supported platform contract, and everything before the
tag is now done.** The source declares `0.2.0`, `CHANGELOG.md` carries a whole-line `## 0.2.0-rc.1`,
and on ad5a71d `ci`, `app` and two release dispatches are all green — all four RIDs packaged in both,
`osx-arm64` included. **The row still closes on a prerelease tag and not before**, because the defect
V-R.0a's last sentence names is the desktop one and `app.yml` sets its suffix only on a tag: both
dispatches logged `prerelease suffix: <none>`. **What a tag needs that this repository does not yet
have is the three R2 secrets**, which it holds none of; the guard refuses a tag without them and now
asks R2 whether the ones it is given actually work (D-0112). Every repair that can be finished
without new hardware is finished. F.2b2 is BLOCKED on a macOS machine and a Linux desktop
session; it is deferred to the desktop candidate run, and 4.7b lists it, so the desktop gate still
cannot pass without it.

**The first push after 2026-09-07 found three defects that no local suite could see, and they are
worth stating because they change what a green local run means.** `main` was red from 1e5abba
onward, so F.1a–F.4a all recorded their evidence from this Windows machine alone; ci run
34305202637 is the first time any of them passed on Linux and macOS. Of the three, two were in the
release path itself: three gate scripts were committed non-executable and died `Permission denied`
on every non-Windows runner (D-0105), and F.4a's own destination check still fell open on
ubuntu-22.04, the runner the publish job uses, because it trusted `jq -e` to exit nonzero on empty
output and jq 1.6 does not (D-0106). The third was a Windows save contending with its own
Transactional NTFS temporary directory (D-0107). Release foundations follow: R.0a makes release
identity and supported targets executable, R.0b/3.8 make distribution verifiable, and 4.7a prepares
desktop packages without waiting for signing enrollment. A new CLI/MCP patch requires the
...
died with `a name that is reserved for use by another transaction`, on a commit that changed two
markdown files and nothing else. The four ci runs before it were green on the same job. D-0107 asked
for this to be watched rather than grown again, so the next move is the cause and not a ninth
attempt. **What that cause is, is now an open question rather than a settled one.** D-0107's account
— that a directory enlisted in one transaction refuses operations from outside it — is refuted by
[scripts/txf-probe.cs](../scripts/txf-probe.cs) on this machine, along with per-process `TEMP`
isolation, duplicate temporary names and 8.3 aliasing, and 6,000 concurrent transacted saves through
one shared `%TEMP%` refuse nothing at all. The budget stands; only its stated reason is withdrawn
(D-0114). F.6 owns the diagnosis and carries what is excluded; the probe on `windows-2025` is its
next action, because the operating system is the remaining difference. **The probe also found a
defect that does reproduce here**, at a different call site: a save refused the vault's own name
throws where the retry does not look, so it fails on the first attempt and strands a file. That is
F.7, kept separate so one ID does not mean two things.
...
| 2.6 | Complete in source | Client setup command, newer than v0.1.0; [CHANGELOG](../CHANGELOG.md#020-rc1) |
...
| F.3d | Complete | A release too large for one frame is refused where it is encoded, delivered on the connection that stays open, and recorded as a release that was approved and could not be sent rather than as an approver that could not be asked; [protocol tests](../tests/Keypaste.Core.Tests/ApproverProtocolTests.cs) for the total encode, the value that never leaks a byte and the authority the refusal names, [framer tests](../tests/Keypaste.Core.Tests/MessageFramerTests.cs) extended to credential replies, [handler tests](../tests/Keypaste.Core.Tests/ApproverHandlerTests.cs) and [policy tests](../tests/Keypaste.Core.Tests/ApproverHandlerPolicyTests.cs) for the single prompt and the operator's terminal, [listener tests](../tests/Keypaste.Core.Tests/ApproverListenerTests.cs) for the surviving connection and its grants, and [large-credential tests](../tests/Keypaste.Mcp.Tests/LargeCredentialTests.cs) over a real vault, a real pipe and a real MCP client; twenty red first, three of them reading back "Something went wrong while asking a person" and one counting three prompts for two requests; 1,174 passed and five platform skips after, with `verify-approval-e2e.sh`, `verify-mcp-stdio.sh`, `verify-policy-e2e.sh` and `verify-log-chain.sh` re-run green; D-0103 |
| F.4a | Complete | A release destination is refused unless a listing positively verifies it is empty: the decision and the upload are one file, [publish-release.sh](../scripts/publish-release.sh), so the fixture drives what the release runs and the workflow step is one line. It reads an `s3api list-objects-v2` body rather than the emptiness of `aws s3 ls` output, and it refuses unless a listing of an already-published prefix comes back non-empty — a nonzero exit alone was the assumption that failed. [verify-release-destination.sh](../scripts/verify-release-destination.sh) runs 24 cases against a fake AWS CLI with no credential and no bucket: one verified-empty destination uploaded 11 files, while occupied, denied, denied-without-a-word, throttled, unreachable, nine malformed shapes, an all-empty impostor and six unusable version strings each made zero write calls and left the seeded objects byte-identical — counted from an allowlist of read verbs and a hash of the fake bucket, not from an exit code. The pre-fix one-liner, same harness and same fake, uploaded over a published version on two of them; weakening the positive control or dropping `jq -e` turns named cases red. [Tripwire tests](../tests/Keypaste.Core.Tests/ReleaseDestinationIsCheckedTests.cs) hold the wiring and go red when the suppression is put back. Run in `ci.yml` and in the `release.yml` guard job, so a dispatch exercises it. The real-awscli phase is now observed: ci run 34305202637 on ubuntu-24.04 reports `aws-cli/2.36.35` and both unreachable endpoints refused, and the `release.yml` guard runs it again on ubuntu-22.04. **Running it also found that this repair was incomplete** — on jq 1.6, which ubuntu-22.04 ships, an empty answer about the destination alone passed the positive control and reached an upload; repaired and given the two fixture cases that reach past the probe, 26 cases now (D-0106). Still not observed: R2's actual reply to a listing, which only a tag can produce; D-0104, D-0106 |
| F.4b | Complete | Every binary keypaste ships reports keypaste as its publisher, and the vendored one reports upstream. [Attribute tests](../tests/Keypaste.Core.Tests/PublisherMetadata.cs) compiled into all four front-end suites assert the built assembly on every operating system; [verify-publisher-metadata.sh](../scripts/verify-publisher-metadata.sh) reads the packaged bytes, including the NativeAOT binaries that have no managed assembly left to reflect over. Observed on the machine that built each target: release dispatch 34305439149 reports `2 binaries publish as keypaste, with upstream's notice intact` for the CLI/MCP NativeAOT builds on **win-x64, linux-x64, linux-arm64 and osx-arm64**, and app dispatch 34302852836 reports `3 binaries` for the desktop packages on **win-x64, linux-x64 and osx-arm64**; ci run 34305202637 carries the attribute half on three operating systems. Windows version resources were read by hand on Windows 10 Pro 19045 (2026-09-08) and `KeePassLib.dll` reports Dominik Reichl. Restoring a personal name fails both halves. Forward-only: `v0.1.0` is published and immutable. The gate had never once run on a runner before this — it was committed non-executable, and both workflows invoked it directly (D-0105); D-0097 |
| F.5 | Complete, on weak evidence | A `tools/call` pipelined behind `notifications/initialized` is no longer refused as a client that never introduced itself. [McpAudit.AwaitCompletionAsync](../src/Keypaste.Mcp/McpAudit.cs) re-reads the identity for a bounded second before refusing, so a client whose `initialize` is still being processed is waited for and one that sent nothing is still refused — the guarantee is unchanged, a request is never answered for a client that cannot be named. [Grace tests](../tests/Keypaste.Mcp.Tests/HandshakeGraceTests.cs) drive a predicate that flips the way `ClientInfo` does; deleting the wait turns two of them red. **Observed on the target it failed on:** `osx-arm64` green in release dispatches 34401987162 and 34402662140, both on ad5a71d, with `verify-approval-e2e.sh` and `verify-policy-e2e.sh` run in that job — green twice, because twice is what it took to see it fail. **Two residuals are open rather than closed, and the row says so instead of reading as proof.** V-F.5's first sentence asks for a reproduction that pipelines all three messages in one write and is red before the fix; no such test exists, because every in-process test drives a real `McpClient`, which waits for the handshake response — the red-before evidence is dispatch 34387033087 itself, which failed twice in two attempts. And the one-second grace is asserted to be large next to thread scheduling and has never been measured against the gap on that runner. **A third occurrence re-opens this row rather than widening the grace**, since a budget grown to fit each new observation is not a fix; D-0113 |
...
- [ ] **F.7 — A save refused the vault's own name gives up on the first attempt.** Needs: 0.2.
  **Build:** When `MoveFileTransacted` is refused, [TxfMove](../third_party/KeePassLib/Serialization/FileTransactionEx.cs) falls back to two non-transacted `MoveFileEx` hops — the temporary file onto the vault's drive, then onto the vault — and throws `Win32Exception` if either fails. [KeePassInterop.IsTransient](../src/Keypaste.Core/Internal/KeePassInterop.cs) accepts `IOException` and `UnauthorizedAccessException` and nothing else, so that failure is not transient, **none of the eight attempts run**, and the save reports failure immediately — while the first hop has already left `<vault>.kdbx.tmp` beside the vault. This is a different defect from F.6 at a different call site with a different exception type, and it is the arrangement keypaste actually ships: CLI, MCP server, approver and desktop all save the user's one vault, and it is the destination name they share. **The exception type does not depend on the operating system and the error code does, which this row says now rather than after a CI run.** `TxfMove` ends `if(!b) throw new Win32Exception();` — unconditional, in the vendored source, so every Windows version raises that type when the fallback move fails; nothing about it is platform-conditional. Whether `windows-2025` reports **6800** for a non-transacted move onto a reserved name is the platform fact, and this machine cannot establish it. So a predicate keyed on the code carries F.6's gap in miniature, and one keyed on the type alone is the wider catch this row already rejects. **It costs no second dispatch:** [txf-probe.cs](../scripts/txf-probe.cs) already reports that code on whatever runner it runs — the `a non-transacted move onto the destination name` line — so the F.6 measurement answers this one too, and the repair waits on the code rather than on the type. **What a different code would mean is settled now rather than when it arrives.** The probe does not infer the condition, it builds it: a live transaction over a known name, then a non-transacted move onto that name. So whatever code that line returns *is* this condition's code on that operating system. A code other than 6800 there is therefore the same failure named differently, the predicate takes both, and each is recorded against the platform that produced it — **the set only ever grows by a code the probe produced under that arrangement.** A code seen in a CI failure does not join it on the theory that it is probably the same thing; that is how the retry budget grew (D-0107), and it gets its own observation. If the move **succeeds** there, nothing reopens and nothing widens: the defect is real on the Windows 10 floor this project advertises and unobserved on Server 2025, and the row says that rather than implying every target. **F.7 was not opened by a CI failure**, so no CI run can reopen it — the dispatch decides only how wide the repair must be. Decide whether the answer is admitting native 6800 to the transient set, cleaning the stranded intermediate file, or both; a wider `catch` that swallows every `Win32Exception` is not it, because a bad path and a full disk must still fail on the first attempt.
  **Verify (V-F.7):** With a real transaction held over the vault's name, a save is retried rather than refused at once, and succeeds if the transaction commits within the budget; no `<vault>.kdbx.tmp` survives a failed save. Red before: reverting the transient rule turns it red, and the failure it reproduces is on record rather than argued. A test that only asserts the exception type does not pass this row — the stranded file is half of it.
  **Status:** Opened 2026-09-09 with a reproduction in hand, which is why it is a row rather than an observation. [scripts/txf-probe.cs](../scripts/txf-probe.cs) reports `a non-transacted move onto the destination name  REFUSED  Win32Exception 6800` on Windows 10 Pro 19045, deterministically, today. **It is deliberately not folded into F.6**, whose Build is refuted and whose Verify is about contention over the temporary name: a repair landing under that ID would leave it meaning two things.

- [ ] **F.6 — Establish why a save is refused a temporary name, then stop it.** Needs: 0.2.
  **Build:** The mechanism this row was opened with is **refuted on Windows 10 Pro 19045**, and the row records that rather than keeping a repair nobody can justify. D-0107, [KeePassInterop.Save](../src/Keypaste.Core/Internal/KeePassInterop.cs)'s remarks and [ConcurrentVaultSaveTests](../tests/Keypaste.Core.Tests/ConcurrentVaultSaveTests.cs) all assert that *a directory enlisted in one transaction refuses operations from outside it*. It does not. `ERROR_TRANSACTIONAL_CONFLICT` attaches to a **name** — Microsoft documents only that, and a probe holding a real KTM transaction open over a move out of a directory confirms it: a fresh file in that directory, a new subdirectory in it, a file in the destination directory and a file in the temp root all succeed, while the moved-out name itself and **its 8.3 alias** each fail 6800. Three candidate explanations for how two savers reach one name are refuted too. **Duplicate identifiers:** `StrTxfTempPrefix` is `KeePass_TxF_`, `StrUtil.AlphaNumericOnly` filters without truncating, so the whole identifier reaches the path, and 80,000 of them across 16 threads collide zero times at 16–22 alphanumeric characters. **Short-name aliasing:** every one of these files shares the stem `KeePas`, so they do compete for `KEEPAS~1..4` and then for hashed `KE<hash>~1` slots, and a reserved alias is genuinely uncreatable — but the reservation exists only between `MoveFileTransacted` and `CommitTransaction`, and for the whole of that window the source file is still visible to outside readers, so the allocator takes the next free slot instead of asking for the reserved one. Commit releases the reservation; closing the handle is not what releases it. **The shared directory itself:** 8 threads × 200 rounds of the exact KeePassLib sequence in one shared `%TEMP%` produced 1,600 commits and zero failures, and three concurrent processes doing the same produced 6,000 and zero. So a per-transaction temporary directory, a lock keyed on the vault path and `UseFileTransactions = false` are all repairs for a cause that is not established, and none of them is taken on this evidence. **The next action is a measurement, not a repair:** run the probe on `windows-2025`, the platform both failures are from, since every mechanism reachable from this machine is now excluded and the OS is the remaining difference. Probe sources are recorded with this row.
  **Verify (V-F.6):** A reproduction saves one vault from several savers at once and is **red on Windows with `SaveAttempts` set to 1**, so the retry cannot absorb what the fix is supposed to remove; green after, with the budget still at 1. [ConcurrentVaultSaveTests](../tests/Keypaste.Core.Tests/ConcurrentVaultSaveTests.cs) says in its own remarks that it is a guard rather than a reproduction and does not fail on a developer machine; replacing that sentence with a test that does is most of this row. **It also never entered the path it claims to guard**, which is why it never reproduced anything: it races eight `Vault.Create` + one `Save()`, and [FileTransactionEx](../third_party/KeePassLib/Serialization/FileTransactionEx.cs)'s constructor forces `bTransacted` false when the base file does not exist, so it executes zero transactions. A green CI run cannot pass this row — the failure is intermittent, which is the whole difficulty — and neither can a repair that only isolates the test harness while two keypaste processes still contend. **A probe workflow is not this verifier and does not conflict with that sentence:** it measures the mechanism on a runner and closes nothing.
  **Status:** Opened 2026-09-09 from ci run 34403613553, which exhausted all eight attempts on `windows-2025` and died with D-0107's exact message on a commit that changed two markdown files. D-0107 asked for the budget to be watched rather than grown again; this is that watch coming back positive, and a ninth attempt is explicitly not the answer. **ci run 34425360000 on the same commit is green — 1,198 tests, 0 failed — and that is the race not firing, not evidence for anything.** The two failures are now read rather than inferred: run 34303291945 `EnvExportTests.AFileBesideTheVault_StillExports` and run 34403613553 `EnvExportTests.TheVaultItself_IsRefused_EvenWithForceAndYes`, both `System.IO.IOException` 6800 naming a `%TEMP%\KeePass_TxF_*.tmp` path, both on a vault `CliHarness.SeedVault` had already created, and in both runs the other two test modules had exited before the failure — so the contending saver was another thread of `Keypaste.Cli.Tests`, in a different class and therefore a different parallel collection. **It sits below F.7 because it waits on a dispatch while F.7 can start today, and it is in no task's Needs — it blocks nothing.** **The Build's mechanism is refuted and the diagnosis is open**; D-0114 records what was excluded and how.
...
- [ ] **R.0a — Pin release identity and the supported platform contract.** Needs: 3.4.
  ...
  **Status:** Built and green locally; **not closed, because nothing here has run on a runner.** [release-targets.json](../release-targets.json) records each advertised target's runner, CPU, archive format, OS floor and **how that floor was established**, separates advertised downloads from the `source_only` RIDs (O-0013), and keeps an append-only `published` record the download pages are held to — never the tag, which is what lets a candidate carry `-rc.1` while README keeps naming 0.1.0. `release.yml` and `app.yml` build `fromJSON` that file, and the publish allowlist and asset count derive from it; the csprojs, README, keypaste.com, `install.yml` and RELEASE's table are held to it. [docs/RELEASE.md](RELEASE.md#floor-evidence) owns what each `floor_evidence` value means and the gate reads that table at run time, refusing a value it does not define and a rule the table does not publish — so `cited`, which is a citation and explicitly not an observation, cannot be read as proof by anyone who never opened the script. macOS and Windows floors are recorded the way `linux-arm64` already was: a value, a citation and a caveat that no run backs it, published verbatim on every page advertising that target. [verify-release-matrix.sh](../scripts/verify-release-matrix.sh) runs 25 fixture cases — all refused, each for the problem it names — plus a negative control that must ACCEPT a null target list, which the real validator refuses. **The gate landed red first and found five defects:** `app.yml` never passed `-p:VersionSuffix=`, so every prerelease tag built a binary reporting the bare version and failed its own check, meaning no desktop candidate could be built at all, which 4.7a needs; `release.yml`'s changelog lookup was an unanchored `grep -qF`, so `## 0.2.0` was satisfied by a `## 0.2.0-rc.1` heading; `site/public/index.html` carried install commands with no sentinels, the one advertising surface nothing version-checked; the tag guard required a successful `ci` run and never an `app` one, so a tag could ship with the desktop gate red or never run, which made "every advertised target has a matching check" false rather than incomplete; and no page stated a macOS or Windows floor at all, which the presence check now refuses. **`published` is held append-only against git history, not against itself** — two fields one commit apart cannot hold each other honest — which needs `fetch-depth: 0`, set on `ci.yml`'s gate and asserted by [tripwire tests](../tests/Keypaste.Core.Tests/ReleaseMatrixIsPinnedTests.cs). Also recorded rather than discovered later: the desktop archives carry a **glibc 2.39** floor against the CLI's 2.35, because `app.yml` builds on ubuntu-24.04 and `release.yml` on 22.04; moving that builder is 4.7a's and verifying it is 4.7b's. **1,186 passed and five platform skips on `keypaste.slnx`**, and each tripwire is falsifiable on its own: deleting the line that derives the required set turns `TheRequiredGates_AreReadFromTheDefinitionAndIncludeTheDesktopGate` red with the repair in its message. `verify-demo.sh` and `verify-release-destination.sh` re-run green. **A runner has now seen every part a runner can see.** `ci` and `app` are green on ad5a71d, and two release dispatches on it, 34401987162 and 34402662140, each packaged all four RIDs — `osx-arm64` included, which is what closed the tag gate F.5 was holding. The version bump rides through: both logged `building 0.2.0-dryrun (suffix='dryrun', publish=false)` and `4 targets:`. **The guard also found a defect of its own on the way, in this gate:** dispatch 34400224237 refused the repository for a changelog lookup `release.yml` has never stopped making, because `code_of ... | grep -q` under `pipefail` reports a present string as absent when the reader exits first and the writer takes SIGPIPE — `ci` had read the same commit correctly an hour earlier, so the gate's answer depended on a race (D-0111). Earlier, `ci` and `app` were green on 71ac604 and the gate ran on `ubuntu-22.04` and `ubuntu-24.04`, its first execution off the Windows machine it was written on. Release dispatch 34387033087 expanded the matrix to `linux-x64`, `linux-arm64`, `osx-arm64`, `win-x64` — the same four — logged `4 targets:` from the digits check, and packaged `keypaste-0.1.0-dryrun-linux-x64.tar.gz`, `-linux-arm64.tar.gz` and `-win-x64.zip` from the derived pattern, with `glibc floor confirmed: Debian 12 yes, Alpine no` behind the one `container-check` floor. The CLI kept its full version through the build (`building 0.1.0-dryrun (suffix='dryrun')`). **The tag guard no longer needs a tag to be exercised:** its decision moved into [require-green-gates.sh](../scripts/require-green-gates.sh), reading the required workflows out of the definition and excluding the release workflow itself, with [verify-green-gates.sh](../scripts/verify-green-gates.sh) driving eight cases through a fake `gh` on every push — both green passes; a missing `app` run, a red one, a commit with neither, a failed call, an empty reply and an unparseable one all refuse — plus a negative control that deletes the one line deriving the set and must then ACCEPT the commit whose desktop gate never ran. Run against the real API it accepts 71ac604 and refuses db3259d, a real commit `app.yml`'s paths filter skipped. **What is still missing is a tag, and the secrets one needs.** `fromJSON` matrix expansion cannot be exercised off one, and an expression that expands to nothing makes GitHub skip the build job, skip publish through `needs:` and report the run green — the digits check that refuses it is asserted by a tripwire and has still never actually run. Closing this row needs a **prerelease tag**, and the list of what only a tag reaches is now short on purpose: the green-gate check, the changelog lookup, the tag-versus-source comparison and the publication allowlist were all extracted into scripts a fixture drives before tagging rather than after (D-0109, D-0110), so what a tag will execute for the first time is the wiring and the desktop suffix path, which is not a decision but a missing build input. **The R2 credentials no longer wait for a tag either:** the guard makes the real listing through `publish-release.sh --check` on any dispatch that has the secrets, so a stale or wrong-bucket credential is refused before the first build rather than after four (D-0112); only their presence is still tag-shaped. [docs/RELEASE.md](RELEASE.md#what-only-a-tag-reaches) owns that list. `osx-arm64` now packages: F.5 is repaired and green twice on ad5a71d, on the weak evidence its row states. **Not held either way:** the live keypaste.com, which deploys by hand with no workflow, so the gate reads the repository's copy and not what is served — R.0b/R.0c own the public origin; D-0108, D-0109, D-0110
