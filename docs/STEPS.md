...
| F.2d | Complete | The master-password field's accessibility surface is a function of the length and not the characters: every peer down the tree, every attached `AutomationProperties.*` and every registered styled property is swept for typed input, for a whole password arriving in one `TextInput`, and for a single keystroke, and two equal-length passwords sharing no character produce an identical surface. Deliberately attaching the fixture, substituting a `TextBox` with `PasswordChar`, substituting a `TextBlock` that draws it, or keeping the characters in a styled property each turn the same assertion red; making the control publish what it is typed turns ten of the seventeen red, observed 2026-09-08. No production change was needed — the mitigation existed and the evidence did not. [automation tests](../tests/Keypaste.App.Tests/Controls/MaskedInputAutomationTests.cs), THREATS T-22; D-0099 |
