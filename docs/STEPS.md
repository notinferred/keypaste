...
The local vault, CLI/env workflow and approval bridge are published as `v0.2.0`, installed and exercised on all four native targets from clean runners. The desktop has partial entry/env screens in source; hosted service, web vault, mobile client and organization credential service remain unimplemented. The save-overrun repair (F.10b) remains open. F.2b2 is BLOCKED on hardware.

F.6, F.8, F.9 and F.11 are closed. F.9 had two worker-pool defects: the bridge's connect deadline could expire before it tried the pipe, reporting a running approver absent, and the test host blocked workers reading its anonymous pipes (D-0129). Both are repaired in `main`; `0.2.0` contains neither repair and still discloses the first defect (D-0126). F.11's torn KDF registry was reached only by in-process test concurrency; KeePassInterop now builds it once before any vault (D-0133). F.10a measured F.10's overrun: a doomed save spends its extra time before the first attempt queued at the in-process save gate behind other saves' key derivation and encryption, while its own attempts cost almost nothing (D-0134). F.10b stops saves that never use the shared temporary directory from queueing.
...
- [ ] **F.10b — Gate only the saves that write into the shared temporary directory.** Needs: F.10a.
  **Measured (D-0134):** the `_saveGate` in `KeePassInterop.Save` serialises every save in the process, but only a transacted save writes into the private temporary directory D-0123 protects. KeePassLib transacts only when the vault file exists, is not a reparse point and TxF is usable. A doomed save whose directory is gone never transacts, yet queued 0–827 ms locally behind other saves' 47–450 ms attempts. [pool-probe run 34804202152](https://github.com/notinferred/keypaste/actions/runs/34804202152) at 6016c88 on `windows-2025` confirmed it: 45 of 48 labelled saves were gate-dominated, with gate wait up to 5012 ms and first attempts ≤5 ms. The queue was behind attempts of up to 3097 ms, and behind one transacted save that held the gate through eight failed attempts and 2.3 s of retry sleeps.
  **Build:** take the gate per attempt, only when that attempt can transact: Windows, and the vault file exists at attempt time. Gating an attempt that then goes non-transacted is safe; skipping the gate for one that transacts is the risk. Record the residual (a vault created between that check and KeePassLib's own) in the remarks. Pin the rule to the vendored `FileTransactionEx` decision with a test that fails if a missing base file ever transacts, as `TheVendoredTemporarySuffixIsStillTheOneWeSweep` pins the suffix. Regression, seconds and in-process: a transacted save whose first attempt is refused (for example, a read-only vault file) blocks in its `SaveWaiting` callback with the gate taken while a doomed save starts. Assert from `SaveTiming` that its gate wait is under 50 ms and that it names no holder. Keep `SaveTimingTests`' held-gate case on a transacted waiter so the instrument proof survives. Raising `SaveAttempts`, `SaveRetryDelayMilliseconds` or the test bound is not a repair; nor is dropping gate wait or attempt work from a claim about total caller wait.
  **Verify (V-F.10b):** the regression is red before the change (gate wait ≈ the held time, holder named) and green after, locally on Windows. Concurrent transacted saves stay serialised: V-F.6 (`ConcurrentVaultSaveTests`), `VaultSaveUnderATransactedNameTests`, `VaultConcurrentWriteTests` and the KeePassXC write-back gate pass. One confirming pool-probe on `windows-2025` records every doomed save's decomposition. These close aggregate V-F.10.
...
- [ ] **4.7a1 — Package an internal Windows installer.** Needs: R.0a, F.4b.
  **Build:** extend `app.yml`'s `win-x64` job to wrap the published self-contained payload in a per-user installer, recording the installer tool and its pin as a decision. Declare the installer in [release-targets.json](../release-targets.json), carry the tag's prerelease suffix into the installer version and file name, and mark the artifact internal and unsigned in its name and metadata. Signing (3.6b) and installation (4.7b) stay out of scope.
  **Verify (V-4.7a1):** a candidate-tag run uploads an installer whose version, suffix and keypaste publisher metadata match the definition, and `verify-release-matrix.sh` refuses an undeclared or mislabelled Windows package. Extracting the installer without running it yields the payload that passes `--selftest`.
- [ ] **4.7a2 — Package an internal macOS bundle and DMG.** Needs: R.0a, F.4b.
  **Build:** extend `app.yml`'s `osx-arm64` job to assemble `Keypaste.app` with an `Info.plist` carrying the full version and keypaste identity, and place it in a DMG. Declare both in [release-targets.json](../release-targets.json), preserve the prerelease suffix and label the artifact internal and unnotarized. Signing and notarization (3.5b) and installation (4.7b) stay out of scope.
  **Verify (V-4.7a2):** a candidate-tag run uploads a DMG whose mounted bundle reports the definition's version and publisher, and whose executable passes `--selftest` from inside the bundle; `verify-release-matrix.sh` refuses an undeclared or mislabelled macOS package.
- [ ] **4.7a3 — Package an internal Linux AppImage.** Needs: R.0a, F.4b.
  **Build:** extend `app.yml`'s `linux-x64` job to wrap the published self-contained payload in an AppImage whose desktop entry and metadata carry the full version and keypaste identity, recording the AppImage tool and its pin as a decision. Declare the AppImage in [release-targets.json](../release-targets.json), preserve the prerelease suffix in its version and file name and label it internal and unsigned. Installation (4.7b) stays out of scope.
  **Verify (V-4.7a3):** a candidate-tag run uploads an AppImage whose embedded version, suffix and publisher match the definition, and whose extracted payload passes `--selftest`; `verify-release-matrix.sh` refuses an undeclared or mislabelled Linux package.
