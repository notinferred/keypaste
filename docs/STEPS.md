...
`0.3.0` publishes the local vault, the CLI/env workflow and the approval bridge on all four native targets, installed from clean runners and carrying no known defects; `0.2.0` and `0.1.0` stay served with their defects disclosed. The desktop is not publishable yet: its packages build, install, upgrade and uninstall without losing user data on runners, and 4.7c1 built the path that would publish them. 4.8 gave it vault creation in source, on rules it now shares with `keypaste init`, so the remaining gap before an installer is worth shipping is storing an existing secret (4.9). Sharing, receiving, the drop relay and native approval are unimplemented, and F.15 records a documented unlock-screen shortcut that does nothing.

4.9 is the next ready code row, then V.2a, V.2b and V.6. Signing and publication (4.7c2) come last, gated on that desktop work and on the identity it names as its input. The macOS desktop app and all Apple signing are Expansion (D-0201). Closed rows keep their evidence in [Completed steps](#completed-steps); the reasoning is in [DECISIONS](../DECISIONS.md).
...
- [ ] **V.6 — Generate passphrases.** Needs: 4.2.
  **Build:** `PasswordGenerator` gains word-list passphrase generation beside its character recipes: a vendored, pinned word list, a word count with a stated minimum, a chosen separator, and the same cryptographic source the character generator already uses. `keypaste generate --words <n>` and the desktop generator offer it, and the recipe records which kind produced a value. The list's provenance, its size and the entropy per word are stated where a person choosing a length can read them. Nothing else consumes it yet; 5.4a owns the six-word share passphrase.
  **Verify (V-V.6):** core tests show the word list is the pinned one by digest and size, that a generated passphrase draws only from it, uses the requested count and separator, and refuses a count below the minimum; a distribution check over many draws fails on a generator biased by modulo and passes on a rejection-sampled one; two generators seeded from the same entropy source do not repeat across draws; `keypaste generate --words` prints only the passphrase on stdout, and the desktop generator produces one the CLI accepts as an entry password through the shared core.
