...
- [ ] **4.7b — Exercise native desktop installation candidates.** Needs: 4.7a.
  **Build:** a dispatch-only workflow downloads a candidate tag's `app-win-x64` and `app-linux-x64` artifacts and the same tag's CLI/MCP assets, checks each hash and attestation, and installs on a fresh runner: the per-user MSI with `msiexec /i` on `windows-2025`, and the AppImage on `ubuntu-24.04` under Xvfb with only [desktop.md](desktop.md#what-it-needs-on-each-platform)'s Linux prerequisites. Each leg creates a vault with the CLI, starts the installed app, and drives unlock, the entry list, an edit, an env run and an approval through the screens the build contains, reporting JSON lines as F.2b2's observer does (D-0146, D-0199). Uninstall and upgrade stay 4.7d.
  **Verify (V-4.7b):** a run against the `v0.3.1-rc.1` candidates records per target that the installed app rendered its first window, opened the CLI-made vault, saved an edit the CLI reads back, ran an env and completed an approval, and that the retained install prompts or their absence are recorded; a candidate whose hash or attestation differs is refused before installation, and a missing prerequisite fails as unreached rather than passing.
