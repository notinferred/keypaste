...
**Next: R.0b — publish complete, immutable component releases.** R.0a is done, so what keypaste
ships is now one checked file the workflows build from rather than a list in seven places. Every
...
| R.0a | Complete | One checked definition answers for every target: [release-targets.json](../release-targets.json) records each advertised target's runner, CPU, archive format, OS floor and **how that floor was established**, separates advertised downloads from the `source_only` RIDs (O-0013), and keeps an append-only `published` record the download pages are held to — never the tag, which is what lets a candidate carry `-rc.1` while README keeps naming 0.1.0. `release.yml` and `app.yml` build `fromJSON` that file, and the publish allowlist and asset count derive from it; the csprojs, README, keypaste.com, `install.yml` and RELEASE's table are held to it. [verify-release-matrix.sh](../scripts/verify-release-matrix.sh) runs 19 fixture cases — all refused, each for the problem it names — plus a negative control that must ACCEPT a null target list, which the real validator refuses. **The gate landed red first and found exactly three defects:** `app.yml` never passed `-p:VersionSuffix=`, so every prerelease tag built a binary reporting the bare version and failed its own check — no desktop candidate could be built, which 4.7a needs; `release.yml`'s changelog lookup was an unanchored `grep -qF`, so `## 0.2.0` was satisfied by a `## 0.2.0-rc.1` heading; and `site/public/index.html` carried install commands with no sentinels, the one advertising surface nothing version-checked. All three repaired and re-verified: `0.1.0-rc.1` composes through `dotnet msbuild -p:VersionSuffix=rc.1`, and the anchored match refuses the rc heading while still finding `## 0.1.0`. **`published` is held append-only against git history, not against itself** — two fields one commit apart cannot hold each other honest — which needs `fetch-depth: 0`, now set on `ci.yml`'s gate and asserted by [tripwire tests](../tests/Keypaste.Core.Tests/ReleaseMatrixIsPinnedTests.cs). Also recorded rather than discovered later: the desktop archives carry a **glibc 2.39** floor against the CLI's 2.35, because `app.yml` builds on ubuntu-24.04 and `release.yml` on 22.04; 4.7b must verify it. **1,185 passed on `keypaste.slnx`** (803 Core, 319 Cli, 68 Mcp; five platform skips) and **1,381 across all five projects**, adding the 173 App and 18 Consistency tests that live in neither solution. Each of the five tripwires is falsifiable on its own: deleting `fetch-depth: 0` from `ci.yml`'s gate, or the digits check from `release.yml`'s targets job, each turns a named test red with the repair in its message. **That second tripwire was missing until it was looked for** — the empty-matrix fail-open the derived matrices rest on was asserted nowhere, and deleting those four lines left the gate at exit 0 and every test passing. A step body is the one thing the fixture harness cannot drive, so it needs a test. **A second check was binding the wrong thing:** the changelog claim asked whether `grep -qxF` appeared anywhere in `release.yml`, which the publish allowlist independently satisfied, so the lookup could be weakened back to `grep -qF` and stay green. It is scoped to the line that reads CHANGELOG.md now, and both workflow checks read instructions with comments stripped, so a sentence naming a flag cannot stand in for using it. All four are falsified against the real workflows rather than a fixture copy. **Not held:** the live keypaste.com, which deploys by hand with no workflow, so the gate reads the repository's copy and not what is served — R.0b/R.0c own the public origin. `--with-public-origin` runs in `install.yml` alone, telling an unreachable origin from a missing asset by positive control; D-0108 |
...
- [x] **R.0a — Pin release identity and the supported platform contract.** Needs: 3.4.
