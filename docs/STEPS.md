...
This revision marks V.3b complete and selects no further implementation task. The founder decides what gets built next. On a later instruction to build the next task, follow the track order and the first ready code row; a specific task selection takes precedence. No backlog item is automatically eligible.
...
|---|---|---|---|
| T1 | Everyday vault use and recovery | A person opens or creates a supported vault, organizes and uses credentials, restores an edit or deletion, recovers a backup and changes vault access settings from the app. | Creation, editing, history, generators and deletion recovery exist; whole-file backups, organization controls, access settings and current-password reveal remain. |
...
Recovery comes before broader feature depth. V.3a repaired the delete that lost an entry and its history and V.3b made that recovery reachable; what remains is the file-level recovery a recycle bin cannot give, and the organization and access settings daily use needs. These five tasks are the next detailed implementation candidates, not work started by this document revision. Each uses the existing shared core and traces to PRODUCT §§1, 2 and 5.7.

- [ ] **V.4a — Produce recoverable encrypted vault backups.** Needs: 0.2.
...
- [ ] **V.4b — Restore and export an encrypted vault from the app.** Needs: V.4a, 4.8.
