...
The local vault, CLI/env workflow and approval bridge are published as `v0.2.0`, installed and exercised on all four native targets from clean runners. The desktop has partial entry/env screens in source; hosted service, web vault, mobile client and organization credential service remain unimplemented. Releasing the save gate across retry sleeps (F.12) remains open. F.2b2 is BLOCKED on hardware.

F.6, F.8, F.9, F.10 and F.11 are closed. F.9 had two worker-pool defects: the bridge's connect deadline could expire before it tried the pipe, reporting a running approver absent, and the test host blocked workers reading its anonymous pipes (D-0129). Both are repaired in `main`; `0.2.0` contains neither repair and still discloses the first defect (D-0126). F.11's torn KDF registry was reached only by in-process test concurrency; KeePassInterop now builds it once before any vault (D-0133). F.10a measured F.10's overrun: a doomed save spends its extra time before the first attempt queued at the in-process save gate behind other saves' key derivation and encryption, while its own attempts cost almost nothing (D-0134). F.10b gates only attempts over an existing vault file, which are the only ones that transact, so doomed and first saves no longer queue (D-0136). A transacted save still holds the gate through its retry sleeps; that is F.12.
...
| F.10b | Only an attempt over an existing vault takes the save gate | [ADoomedSave_DoesNotQueueBehindATransactedSave](../tests/Keypaste.Core.Tests/SaveTimingTests.cs) red at 1287 ms of gate wait behind its holder, green ungated; [pin](../tests/Keypaste.Core.Tests/VaultSaveUnderATransactedNameTests.cs) on the vendored rule; closes F.10, D-0136 |
...
- [ ] **F.12 — Release the save gate across retry sleeps.** Needs: F.10b.
  **Measured:** [pool-probe run 34804202152](https://github.com/notinferred/keypaste/actions/runs/34804202152) at 6016c88 on `windows-2025` recorded a 5012 ms gate wait. Of that, 4580 ms was one transacted save (op 12, pid 8796) holding the gate through eight failed attempts of 212–432 ms each and 2.3 s of retry sleeps; a single attempt elsewhere took 3097 ms. F.10b left doomed and first saves outside the gate, but a transacted save still holds it from its first attempt to the end of its save, sleeps included (D-0136).
  **Build:** release the gate before each retry wait and re-read, and take it again before the next attempt that can transact, so transacted attempts stay serialised (D-0123) while sleeps no longer are. Preserve the re-read across every wait (D-0119) and the stranded-temporary sweep. The held-gate proof in `SaveTimingTests` holds the gate from inside a retry wait, which this change ends; replace it with a holder that is inside an attempt before relying on it. Regression, seconds and in-process: a transacted waiter starts while a transacted holder blocks in its retry wait; its gate wait must fall from about the held time to under 50 ms.
  **Verify (V-F.12):** the regression is red before and green after on Windows; V-F.6, `VaultSaveUnderATransactedNameTests`, `VaultConcurrentWriteTests` and the KeePassXC write-back gate pass; one confirming pool-probe on `windows-2025` shows no gate hold spanning a retry sleep.
