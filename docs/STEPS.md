...
credential service exists. Three repairs are open — F.6 on the Windows save path, whose cause is now
measured and recorded in D-0122 and whose repair waits on one further probe result, F.8 on one
unexplained listing result, and F.2b2, BLOCKED on hardware rather than on work. **F.6 is next.**
...
- [ ] **F.6 — Establish why a save is refused a temporary name, then stop it.** Needs: 0.2.
  **Build:** **The cause is established.** A transaction holding a `KeePass_TxF_*` name also reserves its 8.3 alias `KEEPAS~1.TMP`; on build **10.0.26100** the short-name allocator then refuses every other `KeePass_TxF_*` create in that directory instead of advancing to the next free alias, which **10.0.20348** does. [txf-probe.yml](../.github/workflows/txf-probe.yml) run 34619717595 measured **1387 of 1600 concurrent saves refused with `IOException` 6800 on `windows-2025` against 0 of 1600 on `windows-2022`**, with `fsutil 8dot3name query C:` reporting 8.3 creation **enabled on both** — so the setting is not the difference, the OS build is. Every name [TxfPrepare](../third_party/KeePassLib/Serialization/FileTransactionEx.cs) builds shares that stem, and `PwDefs.ShortProductName` is `KeePass`, so two keypaste processes — or keypaste and KeePass 2 — collide in one `%TEMP%`. D-0122 owns the measurement and corrects D-0114's exclusion of 8.3 aliasing, which held only for the machine it was measured on. **The reservation is directory-scoped, so the repair is a per-save temporary directory:** run 34620670405 committed **1600 of 1600 on `windows-2025` when each saver had its own temporary directory, against 363 of 1600 sharing one**, and a `KeePass_TxF_*` name in any other directory was accepted while one was held. [TxfPrepare](../third_party/KeePassLib/Serialization/FileTransactionEx.cs) takes its directory from `UrlUtil.GetTempPath()`, so keypaste steers that with a directory private to the process (D-0123) rather than editing vendored source (D-0120’s boundary). `UseFileTransactions = false` stays rejected on D-0107's reasoning, and more retries cannot be the fix when V-F.6 pins the budget at 1.
  **Verify (V-F.6):** [ConcurrentVaultSaveTests](../tests/Keypaste.Core.Tests/ConcurrentVaultSaveTests.cs) saves a vault while **other processes hold `KeePass_TxF_` names in the shared `%TEMP%`**, and is **red at `SaveAttempts = 1`** on `windows-2025` before D-0123 and green after, with the budget still at 1. The contenders are [Keypaste.TxfContender](../tests/Keypaste.TxfContender/Program.cs), which references no keypaste assembly: D-0122 measured the cause as *any* holder of such a name, so KeePass 2 saving its own database is the model, and a contender carrying the fix would contend with nobody. They hold until the test releases them, so the refusal is met every run rather than most. The budget is 1 because eight attempts absorb this refusal — which is what made the original failure intermittent — so a green run at eight would only prove the budget outlasted the contention. The save redirects through the same call path a shipped binary uses and the test adds no setup of its own; a green run must prove the product, not the harness. Where the build hands out the next 8.3 alias instead of refusing, the test skips and says so, because there is nothing there to survive.
  **Note:** Opened from ci run 34403613553, which exhausted all eight attempts on `windows-2025` on a two-markdown-file commit; both failures read from `EnvExportTests`, `IOException` 6800 on a `%TEMP%\KeePass_TxF_*.tmp` path. **It is in R.1's Needs**: an intermittent save failure on a mainstream consumer OS is what that gate exists to catch. **ci run 34610451134 is a second sighting on the create hop**: `VaultSaveUnderATransactedNameTests.ANameThatComesFreeAgain_IsSavedRatherThanRefusedAtOnce` failed with `IOException` 6800 on `C:\Users\runneradmin\AppData\Local\Temp\KeePass_TxF_vXu98mkl4RgRQbzsQm8ECA.tmp` on a commit touching only documents and `release-targets.json`, and ci run 34614262160 on the next commit was green on all three legs — intermittent, not a regression. **That run is this row's evidence and not F.7's**: F.7 holds the vault's own name and observes 6800 on the *move*, this row is 6800 on the *create* of a `%TEMP%` name. Same number, different hop — reading one as evidence for the other is the conflation D-0114 had to undo. **10.0.26100 is Windows 11 24H2 as well as Server 2025, so this reaches shipped `0.2.0` on a consumer desktop; whether it becomes a recorded known defect is open and is the owner's call.**
...
- [ ] **R.1 — Verify the working password manager.** Needs: R.0c, F.6, P.0, 4.7c, 4.7d, 8.4b, K.4, K.5, 4.8, 4.9, V.1b, V.2b, V.3b, V.4b, V.5b, V.6, V.7, V.8b, V.9, 9.1f, 9.2b, 1.4c, 4.4b, 4.3b, E.1, 8.2b, 4.5, 4.6, 9.4, 3.10a. — **Milestone gate:** a fresh user does the whole daily journey on public downloads without terminal help.
