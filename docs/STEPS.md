...
This revision marks V.3a complete and selects no further implementation task. The founder decides what gets built next. On a later instruction to build the next task, follow the track order and the first ready code row; a specific task selection takes precedence. No backlog item is automatically eligible.
...
The desktop has entry and env editing, history, local locking and log reading in source. Its Windows and Linux internal packages have install/upgrade evidence, but there is no public desktop release. Its Agent Activity screen probes a listener; it does not receive or answer credential requests. Its log screen reads records produced elsewhere; it does not establish an integrated approval or revocation path. Env cards copy a command; they do not launch a project through the app's session. A deleted entry can now be recovered in core, but no keypaste interface shows what is in the bin; a history reader/restorer and a recycle bin do not establish encrypted vault backups.
...
|---|---|---|---|
| T1 | Everyday vault use and recovery | A person opens or creates a supported vault, organizes and uses credentials, restores an edit or deletion, recovers a backup and changes vault access settings from the app. | Creation, editing, history, generators and core deletion recovery exist; the trash interface, whole-file backups, organization controls, access settings and current-password reveal remain. |
...
Recovery comes before broader feature depth. V.3a repaired the delete that lost an entry and its history; what remains is the interface that makes the recovery reachable, and the file-level recovery a recycle bin cannot give. These five tasks are the next detailed implementation candidates, not work started by this document revision. Each uses the existing shared core and traces to PRODUCT §§1, 2 and 5.7.

- [ ] **V.3b — Add trash and recovery to the app.** Needs: V.3a, 4.2.
...
- [ ] **V.1a — Support vault access changes in core and CLI.** Needs: 0.2. — Supported KDBX keyfile forms and master-password/keyfile changes with atomic save, old/new unlock verification, and explicit backup-credential consequences; reject unsupported forms without rewriting a vault.
