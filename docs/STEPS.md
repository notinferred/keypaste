...
- [ ] **V.2b — Restore an entry from the desktop.** Needs: V.2a, 4.2.
  **Build:** an entry's detail pane offers its history: the revisions V.2a reads, newest first, each with its modification time, and the fields of a selected revision shown beside the current ones with the password masked until it is held, as an env value is (D-0045). Restoring one calls V.2a's restore and saves through the same core path as an edit, so the replaced value becomes a history item rather than being lost (D-0014), and the screen reloads from the vault afterwards. Nothing is revealed by default, and locking clears the view. [desktop.md](desktop.md) describes the screen.
  **Verify (V-V.2b):** headless tests open an entry with three revisions, show them newest first with their times, restore the oldest and read the current value back through `keypaste get --show`, with one more history item and the UUID unchanged; the vault opens in KeePassXC through the compatibility gate; a revision's password is masked until held and hidden again on release, on switching screens and on lock; an entry with no history says so rather than showing an empty list; and the differential accessibility check (D-0099) holds for every revealed revision field.
