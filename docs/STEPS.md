...
| F.12 | A save sleeping between attempts holds no save gate | [Regressions](../tests/Keypaste.Core.Tests/SaveTimingTests.cs) red then green on Windows 10 19045: a waiter's gate wait 1113 ms, then 0; an in-process commit during the gate wait reverted, then refused. Suite 1,235 passed, 0 failed; confirming pool-probe pending; D-0137 |
