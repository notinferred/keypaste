...
| 3.6b | Windows binaries and MSI sign through a pinned dlib, rehearsed | [sign-windows.sh](../scripts/sign-windows.sh) 24 cases; matrix gate 61. Run 35174017853 at 3b30bd1: payload and MSI accepted under a runner root, Microsoft timestamp; changed byte, no timestamp, outside root, no identity refused; no upload. Run 35174026029 unsigned; D-0204 |
...
- [ ] **4.9 — Enter and edit an existing secret.** Needs: 4.2.
  **Build:** with generation off, Add takes an existing password typed or pasted into a `MaskedInput` whose paste is enabled only for entry secrets, read once from the clipboard into its clearable buffer; the unlock field keeps paste off. The detail pane gains Replace password, written through `Vault.UpdateEntry` so the replaced value becomes a history item (D-0014) under 4.2's lost-write protection. URL and notes stop using `EntryNameSanitizer` for a display rule in `Keypaste.Core` that keeps punctuation, tabs and line breaks, marks bidirectional, zero-width and other control characters, and discloses truncation ([ui-review](ui-review.md#window-and-text-behavior)). [desktop.md](desktop.md) drops its CLI-only limit.
  **Verify (V-4.9):** headless tests add an entry with a pasted and a typed value, save, lock, unlock and reread it, and `keypaste get --show` prints the same value; a replaced password leaves one more history item and the vault opens in KeePassXC through the compatibility gate; the value never reaches a view-model property, binding or the automation tree (D-0099); `https://example.test/path?a=b#c` and ui-review's synthetic notes keep punctuation and line breaks, while U+202E, U+200B and C0 controls other than tab and newline are marked; locking mid-entry clears the buffer.
