...
- [ ] **F.6 — Stop concurrent saves contending for one transacted directory.** Needs: 0.2.
  ...
  **Status:** Opened 2026-09-09 from ci run 34403613553, which exhausted all eight attempts on `windows-2025` and died with D-0107's exact message on a commit that changed two markdown files. D-0107 asked for the budget to be watched rather than grown again; this is that watch coming back positive, and a ninth attempt is explicitly not the answer. **ci run 34425360000 on the same commit is green — 1,198 tests, 0 failed — and that is the race not firing, not evidence for anything.** A failure seen once in five runs is not contradicted by a sixth that passes; the row closes on V-F.6's reproduction being red at `SaveAttempts` 1 and green after, which no green CI run can supply. It is recorded here only so nobody reads the green as the defect having gone.
