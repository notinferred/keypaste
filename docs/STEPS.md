...
weak evidence its row states rather than hides. Three repairs are still open: F.7 and F.6 on the
Windows save path, where the account of the contention was withdrawn rather than fixed (D-0114), and
F.2b2, BLOCKED on hardware rather than on work.
...
**Next: F.7 — a save refused the vault's own name gives up on the first attempt.** It is the first
unchecked row in the earliest unfinished milestone whose Needs are complete (0.2), it reproduces
deterministically on this machine, and it waits on no hardware, credential or dispatch. F.6 sits
behind it because its next action is the `windows-2025` probe. F.2b2 is BLOCKED on a macOS machine
and a Linux desktop session; it is deferred to the desktop candidate run, and 4.7b lists it, so the
desktop gate still cannot pass without it.

**R.0a is where release work resumes, and everything before the tag is done.** The source declares
`0.2.0`, `CHANGELOG.md` carries a whole-line `## 0.2.0-rc.1`, and on ad5a71d `ci`, `app` and two
release dispatches are all green — all four RIDs packaged in both, `osx-arm64` included. **The row
still closes on a prerelease tag and not before**, because the defect V-R.0a's last sentence names is
the desktop one and `app.yml` sets its suffix only on a tag: both dispatches logged `prerelease
suffix: <none>`. **What a tag needs that this repository does not yet have is the three R2 secrets**,
which it holds none of; the guard refuses a tag without them and now asks R2 whether the ones it is
given actually work (D-0112).
...
| F.3c | Complete | One listing reply is bounded by encoded bytes rather than by an entry count, and a reply that left names out says so before the list and after it; [protocol tests](../tests/Keypaste.Core.Tests/ApproverProtocolTests.cs) carry the reproduced 1,000-name case at 76,060 bytes against a 65,535-byte budget and hostile Unicode, with [listing-size](../tests/Keypaste.Mcp.Tests/ListingSizeTests.cs) and [large-vault tests](../tests/Keypaste.Mcp.Tests/LargeVaultListingTests.cs) over a real vault, pipe and MCP client; twenty red first, `verify-mcp-stdio.sh` and `verify-approval-e2e.sh` green; D-0102 |
| F.3d | Complete | A release too large for one frame is refused where it is encoded, delivered on the connection that stays open, and recorded as approved-and-undeliverable rather than as an approver that could not be asked; [protocol](../tests/Keypaste.Core.Tests/ApproverProtocolTests.cs), [handler](../tests/Keypaste.Core.Tests/ApproverHandlerTests.cs) and [large-credential tests](../tests/Keypaste.Mcp.Tests/LargeCredentialTests.cs) over a real vault, pipe and MCP client; twenty red first, three reading back "Something went wrong while asking a person" and one counting three prompts for two requests; the approval, stdio, policy and log-chain gates green; D-0103 |
| F.4a | Complete | A release destination is refused unless a listing positively verifies it is empty, with the decision and the upload in one file, [publish-release.sh](../scripts/publish-release.sh), so the fixture drives what the release runs; [verify-release-destination.sh](../scripts/verify-release-destination.sh) puts 26 cases through a fake AWS CLI, where only the verified-empty destination reaches an upload and every other answer makes zero write calls, and the pre-fix one-liner uploads over a published version on two of them; [tripwire tests](../tests/Keypaste.Core.Tests/ReleaseDestinationIsCheckedTests.cs) go red when the suppression is put back, and ci run 34305202637 observed the real-awscli phase on ubuntu-24.04 with both unreachable endpoints refused. Still unobserved: R2's own reply to a listing, which only a tag produces; D-0104, D-0106 |
| F.4b | Complete | Every binary keypaste ships reports keypaste as its publisher and the vendored one reports upstream, asserted on the built assembly by [attribute tests](../tests/Keypaste.Core.Tests/PublisherMetadata.cs) in all four front-end suites and on the packaged bytes by [verify-publisher-metadata.sh](../scripts/verify-publisher-metadata.sh), which is the only reader a NativeAOT binary has; release dispatch 34305439149 reports `2 binaries publish as keypaste, with upstream's notice intact` across win-x64, linux-x64, linux-arm64 and osx-arm64, app dispatch 34302852836 reports `3 binaries` for the desktop packages, ci run 34305202637 carries the attribute half on three operating systems, and the Windows version resources were read by hand on Windows 10 Pro 19045 (2026-09-08); forward-only, since `v0.1.0` is published and immutable; D-0097, D-0105 |
| F.5 | Complete, on weak evidence | A `tools/call` pipelined behind `notifications/initialized` is waited for rather than refused as a client that never introduced itself, and a client that sent nothing is still refused; [grace tests](../tests/Keypaste.Mcp.Tests/HandshakeGraceTests.cs) go red when the wait is deleted, and `osx-arm64` is green in release dispatches 34401987162 and 34402662140 on ad5a71d against two failures in two attempts of 34387033087. **Weak because two residuals are open:** the reproduction V-F.5 asks for does not exist, since every in-process test drives a real `McpClient` that waits for the handshake response, and the one-second grace has never been measured against the gap on that runner — so a third occurrence re-opens this row rather than widening the grace; D-0113 |
...
- [ ] **F.7 — A save refused the vault's own name gives up on the first attempt.** Needs: 0.2.
  **Build:** When `MoveFileTransacted` is refused, [TxfMove](../third_party/KeePassLib/Serialization/FileTransactionEx.cs) falls back to two non-transacted `MoveFileEx` hops — the temporary file onto the vault's drive, then onto the vault — and throws `Win32Exception` if either fails. [KeePassInterop.IsTransient](../src/Keypaste.Core/Internal/KeePassInterop.cs) admits only `IOException` and `UnauthorizedAccessException`, so **none of the eight attempts run**, the save reports failure at once, and the first hop has already stranded `<vault>.kdbx.tmp` beside the vault. Decide between admitting the native code to the transient set, cleaning the stranded file, or both; a `catch` over every `Win32Exception` is not it, because a bad path and a full disk must still fail on the first attempt. **The type is unconditional in the vendored source and the code is the platform fact**, so key the predicate on the code: F.6's probe run reports it on `windows-2025` at no extra dispatch, from the `a non-transacted move onto the destination name` line, and the probe builds the condition rather than inferring it. Record each code against the platform whose probe produced it; a code seen only in a CI failure gets its own observation rather than joining the set, which is how the retry budget grew (D-0107). If the move succeeds there, nothing widens and nothing reopens — the defect stands on the Windows 10 floor this project advertises and is unobserved on Server 2025.
  **Verify (V-F.7):** With a real transaction held over the vault's name, a save is retried rather than refused at once, and succeeds if the transaction commits within the budget; no `<vault>.kdbx.tmp` survives a failed save. Red before: reverting the transient rule turns it red. A test that only asserts the exception type does not pass this row — the stranded file is half of it.
  **Status:** Opened 2026-09-09 with a deterministic reproduction, which is why it is a row rather than an observation: [scripts/txf-probe.cs](../scripts/txf-probe.cs) reports `a non-transacted move onto the destination name  REFUSED  Win32Exception 6800` on Windows 10 Pro 19045. **Not folded into F.6**, whose Build is refuted and whose Verify is about contention over the temporary name — one ID would then mean two things. It was not opened by a CI failure, so no CI run reopens it; the dispatch decides only how wide the repair must be.

- [ ] **F.6 — Establish why a save is refused a temporary name, then stop it.** Needs: 0.2.
  **Build:** The mechanism this row was opened with — a directory enlisted in one transaction refusing operations from outside it — is **refuted on Windows 10 Pro 19045**, and the row records that rather than keeping a repair nobody can justify. `ERROR_TRANSACTIONAL_CONFLICT` attaches to a **name**, and the three ways two savers could reach one name — duplicate identifiers, 8.3 aliasing and the shared `%TEMP%` directory — are each excluded by probe; D-0114 owns what was tested and how, and the probe sources are recorded with this row. So a per-transaction temporary directory, a lock keyed on the vault path and `UseFileTransactions = false` are all repairs for a cause that is not established, and none is taken. **The next action is a measurement, not a repair:** run the probe on `windows-2025`, the platform both failures came from, since every mechanism reachable from this machine is excluded and the operating system is the remaining difference.
  **Verify (V-F.6):** A reproduction saves one vault from several savers at once and is **red on Windows with `SaveAttempts` set to 1**, so the retry cannot absorb what the fix is supposed to remove; green after, with the budget still at 1. [ConcurrentVaultSaveTests](../tests/Keypaste.Core.Tests/ConcurrentVaultSaveTests.cs) **never enters the transacted path at all**, which its own remarks now state rather than leaving to this row: it races eight `Vault.Create` + one `Save()`, and [FileTransactionEx](../third_party/KeePassLib/Serialization/FileTransactionEx.cs)'s constructor forces `bTransacted` false when the base file does not exist, so it executes zero transactions. Replacing it with a test that does fail is most of this row. A green CI run cannot pass this row — the failure is intermittent, which is the whole difficulty — and neither can a repair that only isolates the test harness while two keypaste processes still contend. The probe workflow is a measurement, not this verifier.
  **Status:** Opened 2026-09-09 from ci run 34403613553, which exhausted all eight attempts on `windows-2025` and died with D-0107's exact message on a commit that changed two markdown files — D-0107 asked for the budget to be watched rather than grown, and this is that watch coming back positive. ci run 34425360000 on the same commit is green, 1,198 tests, which is the race not firing rather than evidence for anything. Both failures are read rather than inferred: run 34303291945 `EnvExportTests.AFileBesideTheVault_StillExports` and run 34403613553 `EnvExportTests.TheVaultItself_IsRefused_EvenWithForceAndYes`, both `System.IO.IOException` 6800 naming a `%TEMP%\KeePass_TxF_*.tmp` path on a vault `CliHarness.SeedVault` had already created, with the other two test modules already exited — so the contending saver was another thread of `Keypaste.Cli.Tests`, in a different parallel collection. It sits below F.7 because it waits on a dispatch while F.7 can start today, and it is in no task's Needs, so it blocks nothing. **The mechanism is refuted and the diagnosis is open**; D-0114.
...
- [ ] **R.0a — Pin release identity and the supported platform contract.** Needs: 3.4.
  ...
  **Status:** Built, and a runner has now seen every part a runner can see; **not closed, because no tag has run it.** [release-targets.json](../release-targets.json) is the one record of what keypaste ships — advertised targets, the `source_only` RIDs (O-0013), each target's runner, CPU, archive format and OS floor with how that floor was established, and an append-only `published` list the download pages are held to rather than the tag — and `release.yml`, `app.yml`, the csprojs, README, keypaste.com, `install.yml` and RELEASE's table all derive from it (D-0108). [docs/RELEASE.md](RELEASE.md#floor-evidence) owns what each `floor_evidence` value means and the gate reads that table at run time, so `cited` cannot be read as an observation by anyone who never opened the script. **Observed:** [verify-release-matrix.sh](../scripts/verify-release-matrix.sh) refuses 25 fixture cases, each for the problem it names, plus a negative control that must ACCEPT a null target list — the 25 is counted from the file's own case lines and matched exactly. [verify-green-gates.sh](../scripts/verify-green-gates.sh) drives [require-green-gates.sh](../scripts/require-green-gates.sh) through a fake `gh` on every push, eight cases with both-gates-green the only accepted shape, and against the real API accepts 71ac604 and refuses db3259d, whose desktop gate `app.yml`'s paths filter skipped. `ci` and `app` are green on ad5a71d, and release dispatches 34401987162 and 34402662140 each packaged all four RIDs, `osx-arm64` included, both logging `building 0.2.0-dryrun (suffix='dryrun', publish=false)` and `4 targets:`. Dispatch 34387033087 confirmed the one `container-check` floor: `glibc floor confirmed: Debian 12 yes, Alpine no`. The wiring none of those can reach — `fetch-depth: 0` on the gate job, and the required-gate set being read from the definition — is held by [tripwire tests](../tests/Keypaste.Core.Tests/ReleaseMatrixIsPinnedTests.cs), each falsifiable on its own. **What closes it is a prerelease tag and the three R2 secrets**, which this repository holds none of; the guard refuses a tag without them and now asks R2 whether the ones it is given actually work (D-0112). What a tag reaches first is short on purpose, because the green-gate check, the changelog lookup, the tag-versus-source comparison and the publication allowlist were extracted into scripts a fixture drives (D-0109, D-0110): the `fromJSON` matrix expansion, the digits check that refuses an expansion to nothing — asserted by a tripwire and never yet run — and the **desktop suffix path**, since `app.yml` sets `VersionSuffix` only on a tag and both dispatches logged `prerelease suffix: <none>`. [docs/RELEASE.md](RELEASE.md#what-only-a-tag-reaches) owns that list. The desktop archives carry a **glibc 2.39** floor against the CLI's 2.35 because `app.yml` builds on ubuntu-24.04 and `release.yml` on 22.04; moving that builder is 4.7a's and verifying it 4.7b's. **Not held either way:** the live keypaste.com deploys by hand with no workflow, so the gate reads the repository's copy and not what is served — R.0b/R.0c own the public origin; D-0108, D-0109, D-0110, D-0111, D-0112
