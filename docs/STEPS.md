...
- [ ] **4.7d — Preserve user data through upgrade, uninstall and recovery.** Needs: 4.7a.
  ...
  **Pending:** the harness, the candidates and the offline selftest (37 cases) are built and green locally; the runner evidence needs one `upgrade-desktop.yml` dispatch, which requires the workflow on `main`. Until that run passes on both RIDs this row stays open, and a contradiction opens its own F row.
