...
- [ ] **F.14 — An upgrade that fails while installing files leaves no keypaste installed.** Needs: 4.7a. — Found by 4.7d's interruption probe, run 35278766000 on `windows-2025` (D-0213).
  **Evidence:** with `0.0.1-upgrade` installed, an upgrade to a `0.0.2-upgrade` package whose cabinet cannot be extracted ran `RemoveExistingProducts` and its nested removal to `Return value 0` and `CleanupConfigData(RemovingProduct=1)`, then failed at `Action ended 21:53:59: InstallFinalize. Return value 3` with `status: 1603`. Rollback ran and unregistered only the new product's components, reporting `Error in rollback skipped. Return: 5`, so nothing was installed afterwards: no registration and no `keypaste-app.exe`. The vault, `app.toml`, `recent.toml`, `policy.toml` and the audit chain were byte-identical throughout, so this costs the application, not the user's data.
  **Build:** [Package.wxs](../packaging/windows/Package.wxs) declares `MajorUpgrade` with no `Schedule`, so `RemoveExistingProducts` commits before the new files do. Decide and record the schedule that keeps the previous version installed when an upgrade fails, and change only that.
  **Verify (V-F.14):** the probe's `truncated-cab` method is the regression: before the change it leaves nothing installed, and after it the transaction still fails while `0.0.1-upgrade` is registered and runnable and the user's five files are byte-identical. 4.7d's interrupted check then adopts that method, and the KeePassXC compatibility gate stays green.
