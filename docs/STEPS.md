...
- [ ] **4.7d — Preserve user data through upgrade, uninstall and recovery.** Needs: 4.7a.
  **Build:** a dispatch-only workflow builds, from one commit, an unpublished pair of candidates at two distinct numeric versions by overriding `VersionPrefix`, because D-0139 cannot order two candidates of one numeric version (D-0147). On `windows-2025` it installs the lower MSI per user, creates a vault with history, an `app.toml`, a `recent.toml`, a `policy.toml` and an audit log through the CLI and app, installs the higher MSI over it through `MajorUpgrade`, interrupts a third install, and then uninstalls with `msiexec /x`. On `ubuntu-24.04` the AppImage has no installer, so an upgrade replaces the file in place. Each leg writes a JSON record of the files it hashed before and after each transition.
  **Verify (V-4.7d):** after the upgrade the higher version runs and `keypaste --version` and the app report it; the vault opens with its entries and history intact and still opens in KeePassXC; `app.toml`, `recent.toml`, `policy.toml` and the audit chain are byte-identical and `keypaste log verify` passes; an interrupted install leaves the previous version runnable; uninstall removes the install folder and Start menu shortcut and leaves `~/.keypaste` and the vault byte-identical; a downgrade is refused by the MSI. The real-release upgrade stays a dated observation at 4.7c.
