...
or unreachable one can no longer read as free and be published over (F.4a). The remaining F.2–F.4
repair tasks below are open even though existing tests pass.
...
finished on this machine is finished; the two left both wait on the same push. F.2b2 is BLOCKED on
...
because NativeAOT does not cross-compile. That same run is what observes F.4a's real-awscli phase
and R2's actual reply to a listing. Release foundations follow: R.0a makes release identity and supported
