...
The local vault, CLI/env workflow and approval bridge are published as `v0.3.0`, installed and exercised on all four native targets from clean runners. It carries the F repairs and no known defects; `0.2.0` and `0.1.0` stay served and disclosed. The desktop has partial entry/env screens in source; sharing, receiving, the drop relay, native approval and the team plan remain unimplemented. 4.7a closed with 4.7a3's Linux AppImage, built on the `v0.3.1-rc.1` candidate tag; 3.6b closed on its runner-trusted rehearsal at `3b30bd1`, with real signing waiting on 3.6a's five values; 4.7b closed on the `v0.3.1-rc.2` candidates, installed and driven on fresh `windows-2025` and `ubuntu-24.04` runners with approval still answered at the terminal (D-0205); 4.7d's harness, its three synthetic candidates and its offline selftest are built and green locally, and its runner evidence waits on one `upgrade-desktop.yml` dispatch from `main` (D-0206), followed by 4.7c, 4.8, 4.9 and V.2a; 3.2, the community introduction, is a Human row that Ships after founder daily use of this release. F.2b2 closed on its runner checks on `ubuntu-24.04` and `macos-15`, and the real-desktop record is deferred to Expansion as F.2b3. F.13, which its observer found on Windows, is closed: a pointer move counts as activity only at a screen position the window already saw, a restore forgets that position, and a touch past the deadline locks instead of reviving the session (D-0202).

F.6, F.8, F.9, F.10, F.11 and F.12 are closed. F.9 had two worker-pool defects: the bridge's connect deadline could expire before it tried the pipe, reporting a running approver absent, and the test host blocked workers reading its anonymous pipes (D-0129). Both are repaired and released in `0.3.0`; `0.2.0` contains neither and still discloses the first defect (D-0126). F.11's torn KDF registry was reached only by in-process test concurrency; KeePassInterop now builds it once before any vault (D-0133). F.10a measured F.10's overrun: a doomed save spends its extra time before the first attempt queued at the in-process save gate behind other saves' key derivation and encryption, while its own attempts cost almost nothing (D-0134). F.10b gates only attempts over an existing vault file, which are the only ones that transact, so doomed and first saves no longer queue (D-0136). F.12 releases the gate before each retry sleep and re-reads after taking it again, so a save queued behind an in-process commit is refused rather than reverting it (D-0137).

Release foundations are closed through 3.8 and R.0f. Download pages mark `0.2.0` and `0.1.0` superseded with their defects, and the pages, release definition and keypaste.com advertise `0.3.0`. R.0e corrected live deployment after finding D-0121's workflow had never deployed. The unadvertised `0.2.1-rc.2` is the first release published with a manifest and a build attestation that a logged-out `gh` verifies (D-0138), and `0.3.0` is the first advertised one to carry them; `0.2.0` has none. The unadvertised `0.2.1-rc.3` tag also built the first internal, unsigned Windows MSI (4.7a1, D-0139), kept as a workflow artifact; the unadvertised `0.3.1-rc.1` tag built the first internal, unsigned Linux AppImage with its tool and runtime pinned (4.7a3, D-0203), and published its CLI/MCP with a verified manifest and attestation. R.1 still requires desktop vault creation and secret entry, env mapping with expiry refusal, share and receive through a hosted drop relay that is also self-hostable, native approval and history recovery, with signing and publication of the Windows and Linux desktop apps and the CLI/MCP on all four current targets; the macOS desktop app and all macOS signing are deferred to Expansion (D-0201).
...
| 3.6b | Windows binaries and MSI signed via a pinned dlib, rehearsed | Run 35174017853 at 3b30bd1: payload, MSI accepted (runner root, Microsoft timestamp); changed byte, no timestamp, outside root refused; no identity refused on a copy. Gate: sign-after-package, msi-sign-after-upload, rehearsal-reaches-upload. 35174026029: MSI NotSigned. D-0204 |
...
- [ ] **F.14 — An upgrade that fails while installing files leaves no keypaste installed.** Needs: 4.7a. — Found by 4.7d's interruption probe, run 35278766000 on `windows-2025` (D-0213).
  **Evidence:** with `0.0.1-upgrade` installed, an upgrade to a `0.0.2-upgrade` package whose cabinet cannot be extracted ran `RemoveExistingProducts` and its nested removal to `Return value 0` and `CleanupConfigData(RemovingProduct=1)`, then failed at `Action ended 21:53:59: InstallFinalize. Return value 3` with `status: 1603`. The previous product was already gone by then, and `Error in rollback skipped. Return: 5` is Windows Installer declining to undo part of the transaction, so nothing was installed afterwards: no registration and no `keypaste-app.exe`. The vault, `app.toml`, `recent.toml`, `policy.toml` and the audit chain were byte-identical throughout, so this costs the application, not the user's data.
  **Build:** [Package.wxs](../packaging/windows/Package.wxs) declares `MajorUpgrade` with no `Schedule`, which is `afterInstallValidate`, the earliest: the previous product is removed before a single new file is copied. Schedule it `afterInstallFinalize`, the only schedule under which a failure in InstallFinalize itself rolls back to the previous product; `afterInstallExecute` covers a failure in InstallFiles but not one in InstallFinalize, which is where this failure landed. The cost is disk: the old files stay until the new install commits. Change only that attribute.
  **Verify (V-F.14):** `truncated-cab` is the regression, red at run 35278766000's schedule with nothing installed afterwards. After the change the interrupt-probe reading has the transaction still failing while `0.0.1-upgrade` is registered and runnable, and 4.7d's interrupted check, which adopts the same method over the fixture, has `0.0.2-upgrade` still running with the user's five files byte-identical. The MSI matrix and installer gates and the KeePassXC compatibility gate stay green.
...
### Introduce the released CLI

The F repairs reached users as `0.3.0` before the desktop journey is finished, and the community introduction follows that release and founder daily use (D-0176).
...
Each packaging child uses [release-targets.json](../release-targets.json) through [app.yml](../.github/workflows/app.yml), preserves the prerelease suffix and keypaste publisher, and labels unsigned candidates internal everywhere. A request for a signed package must fail closed when its identity is missing; 3.6b owns signing. An artifact establishes Packaged only; 4.7b installed internal candidates on runners, and public distribution and its installation evidence remain 4.7c.

- [ ] **3.6a — Enable the Windows signing identity (H-0017).** Human. Needs: none. — A Microsoft Artifact Signing identity as keypaste, or an OV certificate in a cloud HSM only if Artifact Signing refuses eligibility (D-0143), repository-scoped with no key retained beyond a job; on `windows-2025`, timestamp a throwaway binary, verify publisher with `signtool verify /pa /v` and reject a changed byte; retain issuer, timestamp authority and expiry, and set the endpoint, account, certificate profile, client ID and tenant ID as the five repository variables 3.6b reads. Enrollment alone does not complete 3.6b.
- [ ] **4.7d — Preserve user data through upgrade, uninstall and recovery.** Needs: 4.7a.
  **Build:** the dispatch-only [upgrade-desktop.yml](../.github/workflows/upgrade-desktop.yml) builds, from one commit, three unpublished candidates at distinct numeric versions by overriding `VersionPrefix`, because D-0139 cannot order two candidates of one numeric version (D-0147); [build-windows-installer.sh](../scripts/build-windows-installer.sh) now owns the MSI build and the definition owns its `tool_sha512` pin (D-0207). [exercise-desktop-upgrade.sh](../scripts/exercise-desktop-upgrade.sh) then installs the lower MSI per user on `windows-2025`, creates a vault with history, an `app.toml`, a `recent.toml`, a `policy.toml` and an audit record through the published CLI/MCP, installs the higher MSI over it through `MajorUpgrade`, interrupts a third install against a volume that does not exist, refuses a downgrade and uninstalls with `msiexec /x`. On `ubuntu-24.04` the AppImage has no installer, so an upgrade replaces the file in place and an interrupted copy is never renamed. Each transition writes a JSON record of the files it hashed before and after it. The harness refuses to start outside CI or a declared sandbox (D-0206).
  **Verify (V-4.7d):** after the upgrade the higher version runs and the installed app reports it, one installation stays registered, and the CLI the fixture used is the published one no upgrade touches (D-0206); the vault opens with its entries and its two history revisions, which KeePassXC also reads; `app.toml`, `recent.toml`, `policy.toml`, the audit chain and the vault are byte-identical and `keypaste log verify` passes; the interrupted install fails and leaves the previous version runnable with the data untouched; uninstall removes the install folder, the Start menu shortcut and the HKCU registration and leaves `~/.keypaste` and the vault byte-identical; a downgrade is refused by the MSI with its downgrade message, which has no Linux equivalent. The real-release upgrade stays a dated observation at 4.7c.
  **Pending:** `linux-x64` has passed twice. Run 35260785520 at `2b91815` failed `win-x64` in setup: a Windows destination path made coreutils escape the digest of the pinned KeePassXC download, so the asset was refused, and the `if: always()` upload then failed the job over an empty directory. One script now owns that download for all three workflows, `fetch-pinned-asset.sh` reads its digest from stdin with a backslashed-path case in its selftest, and the records directory is made before the first step that can fail while an empty upload only warns (D-0208). Run 35263524309 at `d804f6f` reached `win-x64`'s first real reading and contradicted at `upgrade` with 0 registrations. Its artifact settles which: the app reported `0.0.2-upgrade` and the log reads `Windows Installer installed the product … Product Version: 0.0.2 … status: 0`, so the MSI is sound and the query was not — this install registers under HKLM's `Installer\UserData\<SID>\Products`, and `registrations_after_install` was empty in the same run, so the count had never held at `0.0.1`. The harness now asks Windows Installer, counts at install-lower as well, decodes the UTF-16 MSI logs that hid both the rollback and the downgrade message, and uninstalls by product code (D-0209). Run 35267657646 at `96ec849` then passed `linux-x64` and, on `win-x64`, passed install-lower (one registration at `0.0.1`), fixture, upgrade (one registration at `0.0.2`) and data-after-upgrade, with the refused downgrade reading 1603 and its message present and the uninstall leaving no app, shortcut or registration and the data identical. Only `interrupted` is open, and it is unreached, not passed: killing the client `msiexec` at InstallFiles does not abort the transaction, the service finishing the install with no rollback (D-0210). The downgrade and uninstall checks stay blocked behind it, and the offline selftest is 41 cases. `interrupt-probe.yml` and [probe-msi-interruption.sh](../scripts/probe-msi-interruption.sh), this step's diagnosis tooling, measure interruptions against the same candidates: a method counts only when the log names a rollback and the lower version is registered and runnable again (D-0211). Its first reading, run 35271224803 at `57736c9`, found none of three (D-0212): an icacls deny was applied and the install still finished, because the installer writes with SYSTEM privileges; truncating the package failed the operation with 1603 only after InstallFiles and InstallFinalize had returned 1, at Error 1316 re-caching it, leaving nothing installed, no rollback and the user data untouched; and the `/V` process ended was not the one owning the transaction, whose server finished the install. `DisableRollback` is 0 in every log, so the runner permits one. The second reading, run 35278766000 at `266956c`, found the mechanism and a defect. `truncated-cab`, whose surgery was preflighted here on Windows 10 Pro 19045, fails the upgrade inside the transaction at `InstallFinalize. Return value 3` — so that is the interruption 4.7d's check will use — and what it exposed is F.14: nothing is installed afterwards, because `RemoveExistingProducts` had already committed, with the user's five files byte-identical throughout. `kill-owner` is refuted: the log's `MSI (s) (7C:48)` prefix is not an operating-system process, so ending what it resolves to left the install running (D-0213). The interrupted check now installs the third candidate from a copy whose cabinet cannot be extracted and passes only on a log naming the failed action, with `0.0.2-upgrade` still registered and runnable and the data identical; it and the downgrade and uninstall checks behind it wait on one dispatch after F.14's repair (D-0214).
- [ ] **4.7c — Publish and verify signed desktop and CLI downloads.** Needs: 4.7a. Ships after: 3.6b, 4.7b, 4.7d, F.2b2.
  **Build:** give the `app` component an origin and the Windows and Linux desktop packages a place in the publication path `release.yml` already owns: `require-release-assets.sh`, `release-completion.sh`, `publish-release.sh` and the manifest and attestation checked by `verify-provenance.sh`, which today bind only `release.yml`'s CLI assets. Build it against the fake-`aws` harness `verify-release-destination.sh` uses and the fixtures `verify-release-completion.sh` drives, without credentials (D-0148). Publishing to permanent public URLs, re-running the checks against those bytes and recording the first real-release upgrade happen only once the Ships after gates pass.
  **Verify (V-4.7c):** against the fakes, a desktop publication is refused while `signing.policy` is `none` or an asset is unsigned, when the destination is occupied or its listing is ambiguous, when an asset is missing from or extra to the definition, and when a manifest or attestation names another workflow or tag; the positive fixture uploads exactly the declared CLI and desktop assets once. After the gates, the public run records the tag, commit, hashes, URLs and signing identity, and `verify-provenance.sh` passes anonymously at the origin.

### Create a vault and store an existing secret

- [ ] **4.8 — Create a vault from the desktop.** Needs: 0.2, 4.1.
  **Build:** the unlock screen, which today tells a new user to run `keypaste init`, offers Create beside Open. Create asks for a path through the save picker and a master password entered twice in `MaskedInput` fields, refuses an existing file and a mismatched confirmation as `keypaste init` does, creates the file through `Vault.Create` and `Save`, remembers it in `recent.toml` and opens the new session. The CLI and app share the creation rules in `Keypaste.Core`, not two copies (PRODUCT §4.2). The empty state and [desktop.md](desktop.md) describe the new path.
  **Verify (V-4.8):** headless tests create a vault from a fresh `KEYPASTE_HOME` and reach an unlocked, empty entry list; `keypaste ls` then opens the file with the same password, and the file opens in KeePassXC through the compatibility gate; an existing path is refused without the file changing, a mismatched confirmation and an empty password are refused before any write, a cancelled picker writes nothing, and `recent.toml` gains the vault only after a successful create; the differential accessibility check (D-0099) holds for both password fields.
- [ ] **4.9 — Enter and edit an existing secret.** Needs: 4.2.
  **Build:** with generation off, adding an entry takes an existing password and adding an env variable takes an existing value, and an existing entry's password or variable's value can be replaced, each typed or pasted into a `MaskedInput` that reports characters into a view-model `SecretBuffer`, holds no complete password string, and is cleared after save, cancel and lock (SECURITY). `MaskedInput` gains `Ctrl/Cmd+V` for these entry-password and env-value fields only; a paste still arrives as one immutable string, as SECURITY states, and the unlock field stays out of scope. A replaced value is written through `Vault.UpdateEntry` or the env store, so KeePass history keeps the old one (D-0014). The URL and notes display stop using `EntryNameSanitizer` for a rule that preserves ordinary punctuation and line breaks while still rejecting deceptive controls ([ui-review](ui-review.md#window-and-text-behavior)). [desktop.md](desktop.md) and SECURITY's desktop input section describe the new fields.
  **Verify (V-4.9):** headless tests enter a typed and a pasted entry password and env value, save, lock, unlock and reread them, and `keypaste get --show` and a `keypaste run` child read the same values; a replaced password or variable leaves one more history item and the vault opens in KeePassXC through the compatibility gate; the buffers are empty after save, cancel and lock; the differential accessibility check (D-0099) holds for every new secret field, and the unlock field still ignores paste; a regression over `https://example.test/path?a=b#c` and multi-line notes with brackets fails before the display change and passes after, preserving ordinary punctuation and line breaks, while U+202E, U+200B and other deceptive controls in the same fields are still rejected.
- [ ] **V.2a — Read and restore entry history in core.** Needs: 0.2.
  **Build:** `Vault` gains a public read of one entry's history, resolved through the same identity rule as `UpdateEntry` (D-0091), returning each item's index and modification time with its fields behind the same protected-value handling as a current entry. A restore takes an index and puts that revision back through KeePassLib's `RestoreFromBackup` with the vault's history settings, so the value it replaces becomes a history item rather than being lost (D-0014), and it keeps the entry's UUID. Neither changes the file until `Save`. No CLI or desktop surface is added; V.2b owns the desktop.
  **Verify (V-V.2a):** core tests update an entry three times, read its history newest-first, ordered by each revision's modification time, restore the oldest and reread it as current while the replaced value is now the newest history item and the UUID is unchanged; an out-of-range index, a missing entry and an ambiguous name are refused without a write; history stays within `HistoryMaxItems` after a restore; the saved vault opens in KeePassXC through the compatibility gate showing the restored value and the full history.
