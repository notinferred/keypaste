...
- [ ] **3.6b — Sign Windows executables and installers.** Needs: 4.7a1.
  **Build:** a signing script run from `app.yml`'s `win-x64` job and `release.yml`'s `win-x64` build Authenticode-signs and RFC 3161-timestamps `keypaste-app.exe`, the first-party DLLs, the MSI and the CLI/MCP executables through `signtool /dlib` (D-0143), reading the endpoint, account, certificate profile and OIDC client ID from repository variables, and refuses to produce an unsigned payload when `signing.policy` asks for a signature and the identity is absent. **Input:** those four values from 3.6a. Until they exist the same script signs with a certificate generated in the job and trusted only on that runner, `signing.policy` stays `none`, nothing signed that way is uploaded, and the dlib path stays unexercised (D-0144). `verify-release-matrix.sh` currently refuses any `signtool sign` while a policy is `none`, so the gate learns to tell the runner-trusted rehearsal from a policy change.
