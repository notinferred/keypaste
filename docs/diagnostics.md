...
| Save readings | `bash scripts/f9-timeline.sh --saves <iteration-directory>...` | Each labelled save's first interval split by component with the dominant one named, the operations holding the gate while it waited, and a count of gate holds kept through a retry sleep. Lists hold one value per attempt, `-` for an ungated one; older lines without `held` carry one gate taken before attempt `gatedat` (D-0136, D-0137). |
