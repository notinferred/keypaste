...
`keypaste run` closes the vault before starting the child and passes values in the child's environment. In source, the desktop's Run and Open terminal pass a project's set the same way, into the environment of the terminal they open, which hands it to everything run there; the confirmation names the command and directory from `~/.keypaste/projects.json`, which any program running as you can edit, so read it before pressing Start. `keypaste run --session` gets the set from the app or `keypaste agent` after you allow the command and directory it shows; those are the run's own claim, and a program running as you could claim one command and start another (THREATS.md T-30). `keypaste run --token` gets a scoped token's sets from the owner without a prompt, except a protected profile, and a token bundle opens without any vault; `keypaste run` refuses it after its expiry, but whoever holds the bundle and token can decrypt it at any time, so retiring a bundle means rotating its values (THREATS.md T-32). A reference file chooses which values a run injects and may add literals beside them, so `run` uses a directory's `.env.keypaste` on its own only for the project `projects.json` maps there (THREATS.md T-31). Locking the desktop does not stop the child or erase its environment. Process inspection, descendants, crash reporters and application logs can expose them. Keypaste itself writes no environment file; `verify-run-injection.sh` checks that temporary directories remain empty. On Windows, closing the console can terminate Keypaste while leaving the child running. Keypaste forwards supported termination signals and waits for the child without escalating to a hard kill, so a child that ignores termination can keep it waiting.
...
`keypaste share` encrypts one field, or an entry's username, password and URL together (`--field login`), on this machine and uploads only ciphertext; the link carries the key, so anyone holding the link can open it until its views or time run out, and the guarantee holds only while keypaste.com serves an honest viewer (THREATS.md T-33). The metadata `GET` carries a passphrase check, so whoever holds the link can test passphrase guesses offline at PBKDF2-SHA256 (600000) cost without spending a view; keypaste therefore refuses a passphrase under 20 characters, and only a generated one, such as `keypaste generate --words 6`, protects an intercepted link.
