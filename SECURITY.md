...
`keypaste share` encrypts one field on this machine and uploads only ciphertext; the link carries the key, so anyone holding the link can open it until its views or time run out, and the guarantee holds only while keypaste.com serves an honest viewer (THREATS.md T-33).
