...
refusals() {
  ...
  for file in "$@"; do
    ...
    # An MSI signature covers its streams, not unused compound-file sectors, so a byte is changed only in a PE.
    case "$file" in
      *.exe | *.dll)
        copy="$base.changed/$(basename "$file")"
        cp "$file" "$copy"
        printf '\x5a' | dd of="$copy" bs=1 seek=$(($(stat -c %s "$copy") / 2)) conv=notrunc status=none
  ...
  if env -u KEYPASTE_SIGNING_ENDPOINT -u KEYPASTE_SIGNING_ACCOUNT -u KEYPASTE_SIGNING_PROFILE -u KEYPASTE_SIGNING_CLIENT_ID \
