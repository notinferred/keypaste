...
- [ ] **The canonical link is `https://github.com/notinferred/keypaste`** and every post below uses it. The `keypaste` organisation is registered and empty; if the repository moves into it, that is a move to make before a launch, never after, and every URL on both pages and in `Directory.Build.props` moves with it (step 0.4).
