...
- [x] **The canonical link is `https://github.com/keypaste/keypaste`** and every post below uses it. The `keypaste` organisation was registered and the repository moved into it on 2026-09-04, before any post went out; the old owner's URLs redirect, and none remain on a shipped page or in `Directory.Build.props`.
