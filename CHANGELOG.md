...
The desktop's Secrets screen no longer overlaps with an entry open: the search box, Add, Organize and Delete run across the list and the entry's pane, and the dividers sit between the panes instead of over them. The main window can no longer be made narrower than 960 px, the least width at which the screen fits ([F.22](docs/steps/F.22.md)).
