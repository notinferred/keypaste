...
Nothing reaches any of this yet. The desktop controls are the next step and the CLI has no verb for them, so renaming anything today still means KeePassXC. This is source only: there is still no desktop download, and the published CLI is unchanged.
