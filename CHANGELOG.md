...
The words come from EFF's long list of 7,776, vendored into keypaste and pinned by SHA-256; keypaste refuses to generate from a list that is not the pinned one. Each word is worth about 12.9 bits, so six words is about 78 bits and six is the floor, which is the strength a shared file will be protected with. The separator defaults to a full stop rather than the conventional hyphen, and a hyphen is refused: four of the list's words are spelled with one, so a hyphen-joined passphrase cannot be split back into the words it was counted in. Nothing yet consumes a passphrase automatically; sharing is still unimplemented.
