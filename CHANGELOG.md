...
Making a vault with a keyfile, and changing a vault's master password or keyfile, are not implemented anywhere: make one in KeePassXC. `keypaste init` takes no keyfile, the desktop cannot open a keyfile vault yet, and nothing recovers a keyfile you have lost any more than a password you have forgotten. This is source only: there is still no desktop download, and the published CLI is unchanged.
