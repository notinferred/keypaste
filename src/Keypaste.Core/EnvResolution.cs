...
public enum EnvOutcome
{
    ...
    /// <summary>The project exists but has no group for the profile asked for.</summary>
    NoProfile = 12,
...
{
    private EnvResolved(string project, string profile, EnvOutcome outcome, IReadOnlyList<EnvVariable> variables, IReadOnlyList<EnvProblem> problems)
    {
        Project = project;
        Profile = profile;
        Outcome = outcome;
    ...
    public EnvPreview Preview => new(Project, [.. Variables.Select(variable => variable.Key)]) { Profile = Profile };
    ...
    public string Refusal => Outcome switch
    {
        ...
        EnvOutcome.Unusable => $"'{EnvProfileNames.GroupPath(Project, Profile)}' cannot be used: " +
    ...
    internal static EnvResolved Released(string project, IReadOnlyList<EnvVariable> variables, string profile = EnvProfileNames.Default) =>
        new(project, profile, EnvOutcome.Resolved, variables, []);
    ...
    internal static EnvResolved Refused(
        ...
        new(project, profile, outcome, [], problems ?? []);
}
...
public static class EnvResolution
{
    ...
    public static EnvResolved Resolve(Vault vault, string project, string profile, IReadOnlyList<string>? keys, TimeProvider clock)
    {
        ...
        return state switch
        {
            SavedRead.Current => Resolve(entries!, groupPaths!, project, profile, keys, clock.GetUtcNow()),
    ...
    /// <summary>Resolves a project's default profile from one read of a vault.</summary>
    ...
    internal static EnvResolved Resolve(
        ...
        string project,
        DateTimeOffset now) =>
        Resolve(entries, groupPaths, project, EnvProfileNames.Default, now);

    /// <summary>Resolves one profile of a project from one read of a vault.</summary>
    /// <param name="entries">Every entry, read together with <paramref name="groupPaths"/>.</param>
    /// <param name="groupPaths">Every group path.</param>
    /// <param name="project">The project name.</param>
    /// <param name="profile">The profile name.</param>
    ...
    internal static EnvResolved Resolve(
        ...
        string profile,
        DateTimeOffset now) =>
        Resolve(entries, groupPaths, project, profile, keys: null, now);
        ...
        DateTimeOffset now)
    {
        ...
        if (!groupPaths.Contains(group, StringComparer.Ordinal))
        {
            return EnvResolved.Refused(
                project,
                groupPaths.Contains(EnvConvention.GroupPath(project), StringComparer.Ordinal) ? EnvOutcome.NoProfile : EnvOutcome.NoProject,
                profile: profile);
        }
        ...
        if (keys is not null)
        {
            members = [.. members.Where(entry => keys.Contains(entry.Title, StringComparer.Ordinal))];

            foreach (var missing in keys.Where(key => !members.Any(entry => string.Equals(entry.Title, key, StringComparison.Ordinal))))
            {
                problems.Add(new EnvProblem(missing, "is not in this profile's set"));
            }
        }
        ...
        {
            variables = [.. keys.Distinct(StringComparer.Ordinal)
                .Select(key => variables.Single(variable => string.Equals(variable.Key, key, StringComparison.Ordinal)))];
        }

        return EnvResolved.Released(project, variables, profile);
    }
}
