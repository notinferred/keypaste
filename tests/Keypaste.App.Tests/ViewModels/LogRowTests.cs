...
public sealed class LogRowTests
{
    ...
    public void A_person_s_approval_says_for_how_long(int seconds, string result)
    {
        ...
        Assert.True(row.AnsweredOrDoneByYou);
    }

    [Fact]
    public void Every_refusal_is_denied_in_danger()
    {
        var row = Row(Entry("request_credential", "denied", "timed-out") with { Entry = "Work/github", Field = "username" });
        ...
        Assert.False(row.AnsweredOrDoneByYou);
    }
    ...
    [Fact]
    public void Today_shows_the_local_time_and_an_earlier_day_its_date()
    {
        var today = Row(Entry("request_credential", "granted", "prompt"));
        var earlier = Row(Entry("request_credential", "granted", "prompt") with { At = _now.AddDays(-3) });

        Assert.Equal("14:32", today.Time);
        Assert.Equal("Jul 25", earlier.Time);
        Assert.Equal("2026-07-28 14:32:00", today.When);
    }

    [Fact]
    public void A_row_the_chain_does_not_vouch_for_is_marked() =>
        Assert.True(Row(Entry("request_credential", "granted", "prompt"), verified: false).Unverified);
