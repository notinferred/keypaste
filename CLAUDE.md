...
The release procedure and platform/channel support matrix live in [docs/RELEASE.md](docs/RELEASE.md). Published version paths are immutable: `release.yml` refuses to overwrite an existing version. If publication leaves a partial version, use a new version rather than replacing its objects. Verify what can be verified before tagging.
