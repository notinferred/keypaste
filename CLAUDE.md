...
- **Docs-only pushes skip `ci.yml`** via `paths-ignore`. The list is deliberately an allow-to-skip: anything not named there triggers the workflow, so a new file is covered by default. The governance documents are read by no gate at all, which is why they skip it.
