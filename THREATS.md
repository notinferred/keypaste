...
**Proved by.** `MaskedInputAutomationTests`, which types a fixture password into the real unlock screen and then sweeps everything a process on the accessibility bus can ask for — every peer down the tree, its name, help text, item status, item type, automation id, class name and value provider, plus every attached `AutomationProperties.*` and every registered styled property on every element. Typed input, a whole password arriving in one `TextInput` as a paste does, and a single keystroke are covered separately, because a search for the whole password cannot see a per-character leak. **The load-bearing one is differential**: two passwords of the same length sharing no character must produce an identical surface, which is true only if nothing on it is a function of the characters. **What stops the sweep being vacuous is the tests that require it to fail** — deliberately attaching the fixture to an automation property, putting a `TextBox` with `PasswordChar` in its place, putting a `TextBlock` that draws it in its place, and a control that kept the characters in a styled property each run the same assertion and assert that it raises. Making `MaskedInput` publish what it is typed turns ten of the seventeen red, observed on 2026-09-08. `MaskedInput.OnCreateAutomationPeer` remains the implementation. T-25's peer test is for the revealed environment-value control and does not cover master-password input.
